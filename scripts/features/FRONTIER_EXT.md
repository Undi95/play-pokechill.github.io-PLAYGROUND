...
Playwright probes live in the testRoom root (not shipped):
...
```bash
python -m http.server 8765         # from testRoom/
node _probe_<name>.js              # separate terminal
```

The probes load the user's `save.json`. `_probe_cheat_and_gold.js`
backs it up to `save.json.auditbak` before mutating and restores at
the end — manual pre-run backups of other probes are recommended if
they mutate state.
