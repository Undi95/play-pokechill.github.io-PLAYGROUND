...
├─ 2.  Brain / role helpers             meneurLabelFr / zoneLeaderEn / …
...
| `frontierDomeSecret` | Dôme de Combat | Battle Dome | Bracket tournament (4 trainers, pick 2) | 7 | 14 |
| `frontierFactorySecret` | Usine de Combat | Battle Factory | Rental pool; post-win swap | 3 | 6 |
| `frontierPikeSecret` | Reptile de Combat | Battle Pike | 14 rooms × 3-door picker | 1 | 5 |
...
`{ key, labelFr, labelEn, pool: [speciesId…] }`. If the theme maps to an
...
Per user rule **"1:1 Pokechill — if we can't make it work, don't assign
...
created **with no ability pill on the info row** — that's intentional;
...
User feedback: "un talent qu'il ne pourrait jamais avoir"
("an ability it could never have").
...
The intersection minus the normal pool — `__CANONICAL_HIDDEN_ONLY` —
is the hidden-locked set. In `pickAbilityForClone`, we added a gate:
`if (isHiddenOnly(id) && id !== defaultId) return;` — a hidden-only
ability is never pushed into the normal-slot candidate list, unless
it's literally the species' own default entry. The hidden slot path
is untouched (it only ever resolves the species' own `hiddenId`).
...
teams felt equally "competent", which undercuts the sense of progress.
User feedback: the tier ramp should "commence par des équipes nulles
jusqu'à atteindre des teams de synergie inédites dans des séries très
haut niveau".
...
- **Silver** → top-6 (default Phase B diversity).
...
on a separate **dev-only branch** (the `playground` repo — not shipped
...
shipped — they pull the maintainer's local `save.json` and a running
...
- **`teams.js:464`** — `injectPreviewTeam` reads
...
`playground` branch for the full iteration trail (≈100 commits).
