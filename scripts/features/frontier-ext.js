...
`.trim();
  ...
  function simulateNatureFor(pkmnId, ctx) {
    ...
    let physMoves = 0, specMoves = 0, fastMoves = 0, slowMoves = 0, statusMoves = 0;
  ...
  function installEnemyLifeOrbDrainHook() {
    ...
    const readLastMove = () => {
      try { return globalEval("typeof nextMoveWild === 'undefined' ? null : nextMoveWild"); }
      catch (e) { return null; }
    };
    const readHp = () => {
      try { return globalEval("typeof wildPkmnHp === 'undefined' ? null : wildPkmnHp"); }
      catch (e) { return null; }
    };
    const readHpMax = () => {
      try { return globalEval("typeof wildPkmnHpMax === 'undefined' ? null : wildPkmnHpMax"); }
      catch (e) { return null; }
    };
    const writeHp = (n) => {
      try { globalEval("wildPkmnHp = " + JSON.stringify(Number(n))); }
      catch (e) {  }
    };
    window.exploreCombatWild = function () {
      ...
      try {
        ...
        const next = Math.max(1, hp - drain);
        writeHp(next);
      } catch (e) {  }
      return res;
    };
  ...
  function pickItemForClone(realId, moveIds, abilityIds, cloneMon, diff, usedItems) {
    ...
    const ORB_REQUIRES = {
      ...
      toxicOrb: ["guts", "toxicBoost", "marvelScale", "livingShield"],
    };
    ...
    const hasStacking = moves.some((m) => m && move[m] && move[m].buildup !== undefined);
    ...
    const hasSlow = moves.some((m) => m && move[m] && typeof move[m].timer === "number" && move[m].timer > __slowCap);
    ...
    for (const [gemId, t] of Object.entries(GEM_TYPE)) {
      const hasTypeMove = moves.some((m) => m && move[m] && move[m].type === t && (move[m].power || 0) > 0);
    ...
    if (cloneMon && typeof cloneMon.evolve === "function") {
      ...
      } catch (e) { incompatible.add("eviolite");  }
    } else {
      incompatible.add("eviolite");
    }
    ...
    for (const id of Object.keys(item)) {
      if (item[id] && item[id].evo) incompatible.add(id);
    }

    const __fastTimerCap = (typeof defaultPlayerMoveTimer === "number") ? defaultPlayerMoveTimer : 2000;
    const __hasFastMove = moves.some((m) => m && move[m] && (move[m].power || 0) > 0
      && typeof move[m].timer === "number" && move[m].timer < __fastTimerCap);
    if (!__hasFastMove) incompatible.add("quickClaw");
    ...
    if (Math.random() < 0.15) {
      const chaosCands = pool.filter((id) => id !== "flameOrb" && id !== "toxicOrb" && !(item[id] && item[id].evo));
      if (chaosCands.length) return chaosCands[Math.floor(Math.random() * chaosCands.length)];
    }
    ...
    let fastCount = 0, multiHitCount = 0;
    ...
    for (const m of moves) {
      ...
      if (Array.isArray(mv.multihit)) multiHitCount++;
    ...
    const generalPool = pool.filter((id) => {
      ...
      if (typedItems.has(id)) {
        ...
        if (/Gem$/.test(id)) {
          const gemType = Object.entries(GEM_BY_TYPE).find(([k, t]) => k === id);
          if (gemType) {
            const t = gemType[1];
