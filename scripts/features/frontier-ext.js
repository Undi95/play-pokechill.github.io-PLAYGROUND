...
(function () {
  ...
  const FACILITIES = [
    {
      ...
      brain: {
        ...
        nameFr: "Demoiselle du Salon Cathy",
    ...
    },
    {
      ...
      brain: {
        ...
        nameFr: "Maître du Palace Esteban",
    ...
    },
    {
      ...
      brain: {
        ...
        nameFr: "Maîtresse du Dojo Carole",
    ...
    },
    {
      ...
      brain: {
        ...
        nameFr: "Reine du Reptile Charline",
  ...
  // See _TEAM_GEN_AUDIT_V2.md §9 for the full derivation and the old vs
  // new pressure curve.
  function computeRunDifficulty(round, facility) {
    ...
    // enemies already have +10% stats (no more "training dummy" Rattatas —
    // canon Gen 3 Frontier IVs start at 3/31 ≈ 10%, we mirror that).
...
`.trim();
  ...
  function injectStyles() {
    ...
    const css = `
      ...
      .frontier-ext-factory-card .tag-item {
        display: inline-flex;
        align-items: center;
        gap: 0.2rem;
      ...
      .frontier-ext-factory-card .tag-item img {
      ...
      /* Hostile-Pokémon sprite inside the status_species event card.
         Pixel-rendered with a subtle red glow aligning with the
  ...
  function arenaResetState() {
    ...
    // from firing into the NEW combat's state (would instant-KO a fresh
    // enemy with a stale "wildPkmnHp = 0" write).
    try {
  ...
  function arenaRenderJudge() {
    ...
    // combat-exit paths (leaveCombat / abandon / defeat / F5 forfeit /
    // arenaResetState) can clearTimeout it. Without this, a verdict
    // triggered in matchup 3 could fire its wildPkmnHp=0 write 4.8 s
    // later into an ALREADY-CLOSED combat — instant-KO'ing whatever
    // wild the player next encountered.
    if (state.__judgeTimer) { try { clearTimeout(state.__judgeTimer); } catch (e) {} }
    state.__judgeTimer = setTimeout(() => {
      state.__judgeTimer = null;
      // Double-gate on "still in the same arena run" — if the player
      // abandoned or defeated out, don't touch wildPkmnHp.
  ...
  function installArenaSwapFreeze() {
    ...
    window.setWildPkmn = function () {
      ...
      try {
        ...
        // Phase D audit fix (H1) — store the swap-freeze timer handle so
        // combat exit can cancel it. Cancel any prior pending timer first
        // so rapid swaps don't leak stacked callbacks.
  ...
  function installRunLockTooltipHook() {
    ...
    // trapped on them with no way out (× hidden, Escape blocked,
    // backdrop-click blocked). showTeamSizeError / showTiedTeamLostModal
    // plant a `.zdc-error-modal-bypass` element inside their content so
  ...
  // whose canon doesn't include it — which is what the user flagged as
  // "un talent qu'il ne pourrait jamais avoir" (an ability it could never
  // canonically have).
  ...
  function __initCanonicalAbilityPools() {
    ...
    // 1st pass: collect every `.ability` and `.hiddenAbility` reference.
    ...
    // 2nd pass: compute the "hidden-only" subset (hidden somewhere, normal
    // nowhere). These are the ones that fail the legality gate.
  ...
  function __initAbilityPool() {
    ...
    // Re-build canonical pools now that pkmn[] is populated.
  ...
  function pickAbilityForClone(realId, moveIds, cloneMon, diff, abilityGate) {
    ...
    // random type-matched species is canonically wrong (user feedback:
    // "un talent qu'il ne pourrait jamais avoir"). We allow two exceptions:
    // the species' OWN defaultId (some legacy dict entries slot a hidden-
    // locked ability as the default, trust the species-level choice), and
    // the species' OWN hiddenId when the hidden slot is resolving (handled
  ...
  function applyItemBstInflation(clone, itemId, abilityId, moveIds) {
    ...
    // multiplies bst every call. A Machamp with hugePower + guts +
    // flameOrb ended up with atk ≈ 6.6× instead of the intended 3.15×.
    // Fixed by moving the status-triggered bumps into the moveBuff wrap,
    // where they fire exactly once per combat on the state-flag flip.
    ...
    // moveBuff wrap now. See comment at the guts branch above.
    ...
    if (canHaveItem) {
      ...
      if (pickedItem && item[pickedItem]) {
        ...
        if (match && pkmn[match.megaFormId]) {
          ...
          // adjustments we wrote earlier at nature-pick time are now on
          // the OLD bst object (garbage-collected) — the info-row nature
          // pill still reads "adamant" but the bst doesn't reflect it.
          // Matches the tooltip.js:1195-1215 canonical mapping.
  ...
  function destroyAllEnemyClones() {
    ...
    // at setWildPkmn time, but never restored. After destroying the clone
    // pkmn[cloneId] is gone, yet saved.currentPkmn still points at it —
    // ANY vanilla path that reads `pkmn[saved.currentPkmn]` post-combat
    // (tooltip renders, type lookups, signature icons) dereferences
    // undefined and crashes. Null out the dangling reference first so
    // downstream reads get a consistent "no enemy" signal.
    try {
      if (typeof saved === "object" && saved && typeof saved.currentPkmn === "string"
          && /^__zdcEnemy_/.test(saved.currentPkmn)) {
        // Restore the REAL species id if we stashed it on the state —
        // the setWildPkmn wrap records the previous value so we can
        // hand it back cleanly. Fallback: the clone state's realId.
  ...
  function dispatchOnSwitchIn(cloneId) {
    ...
    // own ability sets the matching weather.
    //
    ...
    // PLAYER had just set. Now the rock bump only fires when the
    // ability-rock pair makes sense (drought + heatRock, drizzle +
    // dampRock, etc.), matching the canonical "your own weather lasts
    // longer" design goal.
    try {
  ...
  // before. The previous order meant installCombatHook's onRunVictory
  // chain (which fires from the wrap chain's inner layer) saw a
  // dangling saved.currentPkmn and a deleted pkmn[cloneId] when it
  // tried to snapshot Pike/Pyramid HP or read enemy state. Post-orig
  // ordering keeps the clone state alive for vanilla's own leaveCombat
  // body + any inner wrapper's reads, and only destroys it once
  // everyone's done.
  ...
  function installEnemyContextMoveBuffHook() {
    ...
    window.moveBuff = function (target, buff, mod, turnOverride) {
      ...
      try {
        ...
        // match substring false positives (e.g. a hypothetical future
        // "poisonedTox" variant would have leaked through the old
        // `/burn|…/` loose test and fired every immunity/status-trigger
        ...
        for (const abId of Object.keys(STATUS_IMMUNITY)) {
          ...
          // Phase D audit fix (M6) — exact equality, not substring
          // matching. `buff.indexOf("paralysis") !== -1` would also
          // match "paralysis1" / "limberParalysis" / etc.
        ...
        // landed, not every major status. Previously a flameOrb-burned
        // clone with wonderSkin, when hit by a separate Sleep move,
        // would also have its pre-existing burn wiped (wrong).
  ...
  function enterFactoryPreviewSlot(run) {
    ...
    run.factoryTeam.forEach((rental, i) => {
      ...
      // Initial round-1 rentals roll with no item (intentional — user picks
      // blind from the 6-mon pool pre-round). Post-victory swap can give
      // them an item via confirmFactorySwap; that item now survives every
  ...
  function openFactorySwapModal(facility) {
    ...
    const renderCard = (rental, side, idx, isSelected) => {
      ...
      // `rental.ability` is sometimes an object ref (player-side rentals
      // come through from rollFactoryAbility which returns ability[id])
      // and sometimes a resolved object from our swap-builder. Extract the
      // readable id either way before handing to format().
      const abilityId = !rental.ability ? null
        : (typeof rental.ability === "object" ? rental.ability.id : rental.ability);
      const abilityLabel = abilityId
        ? (typeof format === "function" ? format(abilityId) : abilityId)
        : "—";
      // Phase D — render the held item when present. Item icon sits next
      // to the ability pill so the player sees at a glance "this rental
      // comes with Choice Band" before committing to the swap.
      const heldItemId = rental.item || null;
      const heldItemLabel = heldItemId
        ? (typeof format === "function" ? format(heldItemId) : heldItemId)
        : null;
      const heldItemHtml = heldItemId
        ? `<span class="tag-item"><img src="img/items/${heldItemId}.png" onerror="this.style.display='none'" alt="">${heldItemLabel}</span>`
        : "";
  ...
  function confirmFactorySwap(facility) {
    ...
    // combat engine sees the new species on next launch. The held item
    // (if the incoming rental carried one) TRANSFERS via the preview-slot
    // write — vanilla's injectPreviewTeam copies `.item` into team[slot]
    // at combat start, and the damage formula reads team[slot].item
    // natively (no additional dispatch needed on the player side).
  ...
  function pyramidItemSprite(id) {
    ...
    // know the PNG is present. Custom Pyramid-only items (berries,
    // potions, revives) aren't in `item[]` → return null and the bag
...
<svg viewBox="0 0 16 20" preserveAspectRatio="xMidYMid meet" class="pyr-char">
  ...
  function pyramidResolveTile(facility) {
    ...
    switch (tile) {
      ...
      case PYR_TILES.ITEM: {
        ...
        // the bag, re-roll up to N times until we find something fresh
        // — better UX than making the "Prendre" screen show an item
        // that'd be silently abandoned on click. If every re-roll
        // collides (player owns every held they can find), fall back
        // to a consumable by filtering the pool.
        state.grid[state.playerY][state.playerX] = PYR_TILES.EMPTY;
        ...
        if (isDuplicateHeld(pick)) {
          // All 10 re-rolls still collided — likely player owns every
          // held type. Force a consumable (they always stack).
    ...
    if (mid) {
      ...
      mid.querySelectorAll("[data-pyr-drop]").forEach((btn) => {
        btn.onclick = () => {
          ...
          // decremented by 1, which left stacked consumables (e.g.
          // 2× Baie Ceriz) behind after click → bag still full →
          // picker re-opens and the player has to click again. Per
          // user rule: one click frees the slot immediately.
  ...
  // `item[id].got` — which would make a Pyramid-equipped Leftovers /
  // Quick Claw / etc. feel weaker than intended if the player only
  // has a single copy. Wrap the function so any id currently on a
  // pikeTeam[slot].item inside an active Pyramid run resolves to
  // level 5 (stars / "max level" label / 5).
  function installPyramidItemMaxLevel() {
    ...
    window.returnItemLevel = function (id, mod) {
      if (isPyramidEquipped(id)) {
        ...
        if (mod === "left")  return (window.gameLang === "fr" ? "(niveau max atteint)" : "(max level reached)");
        return 5;
      }
      // Defensive fall-through for custom Pyramid-only ids (berries,
      // potions, revives) not in item[]. Vanilla would throw on
      // item[id].got — return level 1 instead.
      if (typeof item === "undefined" || !item[id]) {
        if (mod === "stars") return `<span style="color:#4fffa7ff">✦</span>✦✦✦✦`;
        if (mod === "left")  return (window.gameLang === "fr" ? "(niveau 1)" : "(level 1)");
        return 1;
      }
      return orig.apply(this, arguments);
    };
  ...
  function renderPikeHpSummary() {
    ...
    for (const sl of ["slot1", "slot2", "slot3"]) {
      ...
      const statusPill = status
        ? `<span class="st ${status}">${statusLabel[status] || status}</span>`
        : "";
      // Heal-on-full glow hint. Slots flagged by applyPikeHealRatio
      // get a short sparkle animation when they sit at 100% HP, so
      // the player understands the heal door DID hit them (UX note:
      // without this, re-healing a full mon looks like a bug).
      // Flag is consumed on render so subsequent renders don't
      // replay the animation.
      // All heal paths set healJustApplied; glow on ANY slot that was
      // just healed (even partial HP, even status-cure only).
      const healFlash = ps.healJustApplied ? " heal-full-flash" : "";
      if (ps.healJustApplied) ps.healJustApplied = false;
      cells.push(`
        <span class="frontier-ext-pike-hp-pill ${cls}${healFlash}">
  ...
  function applyPikeHealRatio(ratio) {
    ...
    for (const sl of ["slot1", "slot2", "slot3"]) {
      ...
      // plays a short glow on slots at 100% HP after a heal door fires.
      // Prevents the "the door healed a full-HP mon? is that a bug?" UX.
  ...
  function showPikeEventModal(facility, kind, title, body, variant, opts) {
    ...
    // modal renders the real sprite (clearer than ☠️ — player spots
    // Gloom / Kirlia / Dusclops immediately). Falls back to emoji when
    // no sprite id supplied OR asset 404s.
  ...
  // previous modal — which for us is typically the locked openSimulatedFight
  // / openFacilityPreview preview that triggered the error. Restoring it
  // pops the player right back into the locked state they just tried to
  // exit (no × button, run-lock active). Empty the whole stack + hide the
  // backdrop directly so Close actually gets the player back to the map.
  // The active run itself is NOT touched — if the player wants to continue
  // the series, they still need to go fix the team in the editor.
  function zdcForceCloseErrorModal() {
    ...
    // Belt+braces — vanilla closeTooltip's animation-then-hide path uses a
    // 150 ms setTimeout to set display:none. If any intermediate state
    // leaves the backdrop visible, force-hide it shortly after.
    setTimeout(() => {
      try {
        const bg = document.getElementById("tooltipBackground");
        if (bg && bg.style.display !== "none") bg.style.display = "none";
        // Also clear panels so nothing lingers visually if the user
        // re-opens a tooltip before the animation settles.
  ...
  function showTeamSizeError(facility) {
    ...
    const mid = document.getElementById("tooltipMid");
    const bottom = document.getElementById("tooltipBottom");
    // Phase D user-reported bug — "continuer une série avec team invalide
    // nous bloque sur cette pop-up". The run-lock hook's isFrontierTooltip()
    // detector scans the WHOLE tooltipBox (including hidden panels) for any
    // frontier-ext-* class — if we leave leftover facility-preview content
    // in tooltipTop/tooltipBottom (just `display:none`, still in the DOM),
    // the lock still applies: × hidden, Escape blocked, backdrop click
    // blocked. Starting a fresh run didn't reproduce because activeRun was
    // null → lock bypassed. Clearing the innerHTML of top/bottom makes this
    // popup look "non-frontier" to the detector, so the × appears. Also
    // surface an explicit Close button in bottom as a secondary escape.
    if (top) { top.style.display = "none"; top.innerHTML = ""; }
    if (titleEl) { titleEl.style.display = "block"; titleEl.innerHTML = "⚠️ " + title; }
    if (mid) { mid.style.display = "block"; mid.innerHTML = `<div class="zdc-error-modal-bypass" style="padding:0.6rem 0.8rem;">${msg}</div>`; }
    if (bottom) {
  ...
  function showTiedTeamLostModal(facility) {
    ...
    const mid = document.getElementById("tooltipMid");
    const bottom = document.getElementById("tooltipBottom");
    // Phase D bug fix — same lock-trap as showTeamSizeError: clearing the
    // panels (not just hiding them) prevents the run-lock hook from seeing
    // leftover frontier-ext-* classes and blocking the ×. Explicit Close
    // button as secondary escape route.
    if (top) { top.style.display = "none"; top.innerHTML = ""; }
    if (titleEl) { titleEl.style.display = "block"; titleEl.innerHTML = "⚠️ " + title; }
      ...
      if (isFactoryFacility(facility)
        ...
        run.pendingFactorySwap = (run.upcomingTrainer.team || []).map((r) => {
          ...
          return {
            ...
            // Life Orb still recoils. Initial round-1 rental pool stays
            // itemless (the player hasn't "earned" them yet), but every
            // post-victory swap is a free item pickup — which creates a
            // nice progression arc across the round: start bare, end in
            // full kit if you swap every fight. Plumbed into
            // enterFactoryPreviewSlot + confirmFactorySwap below.
            item: cache ? cache.item : null,
            // Flavour extras shown on the card but not mechanically
            // transferred — shiny sparkle and hidden-ability unlock stay
            // with the NPC (they're post-Gold boss perks, not rental loot).
  ...
  function bootstrap() {
    ...
    const forfeitOnBoot = () => {
      try {
        ...
        const run = saved.frontierExt.activeRun;
        if (!run) return true;
        // Strip transient healJustApplied glow flag — saveGame
        // serializes run.pikeTeam, so without this scrub an F5 would
        // phantom-glow on first render after load.
        if (run.pikeTeam) {
          for (const sl of ["slot1", "slot2", "slot3"]) {
  ...
  window.__frontierExt = {
    ...
    pickAbilityForClone,
    pickItemForClone,
    // Probe hook: expose the private clone state so automated tests can
    // assert on dispatch-level fields (ability / hiddenAbility / item / shiny)
    // instead of the species-inherited prototype-like fields on pkmn[cloneId].
    __getEnemyCloneState: (cloneId) => __enemyCloneState[cloneId] || null,
    applyBstInflation,
