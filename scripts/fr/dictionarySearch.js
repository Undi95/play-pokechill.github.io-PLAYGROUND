...
fuseDictionary = new Fuse(dictionaryItems, {
  keys: [ 
    ...
    { name: 'renameFR', getFn: obj => obj.renameFR || '' }, 
    'type', 
    'hiddenAbility.id', 
    'split', 
    'tagObtainedIn', 
...
function setSearchTags() {
    for (const e in pkmn){
        if (pkmn[e].caught>0) { pkmn[e].tagCaught = "caught"; pkmn[e].tagCaughtFR = "capture"; }
        if (pkmn[e].shiny) { pkmn[e].tagShiny = "shiny"; pkmn[e].tagShinyFR = "shiny"; }
        ...
        if ( wildlifePoolCommon.includes(e) || wildlifePoolUncommon.includes(e) || wildlifePoolRare.includes(e) ) { pkmn[e].tagObtainedIn = "park"; pkmn[e].tagObtainedInFR = "parc"; }
        if ( exclusiveFrontierPkmn.includes(pkmn[e]) ) { pkmn[e].tagObtainedIn = "frontier"; pkmn[e].tagObtainedInFR = "zdc"; }
        if (pkmn[e].pokerus) { pkmn[e].tagPokerus = "pokerus"; pkmn[e].tagPokerusFR = "pokerus"; }
        if (pkmn[e].movepool && pkmn[e].movepool.length > 0) {
            pkmn[e].movepoolFR = pkmn[e].movepool.map(m => move[m]?.renameFR || move[m]?.rename || m).join(' ');
        }
    }
    ...
    for (const i in move){
        if (move[i].moveset ==undefined ) { move[i].tagSignature = "signature"; move[i].tagSignatureFR = "signature"; }
    }
...
function getAbilityPoolByTier(poke) {
    if (!poke || !poke.type) {
        console.log(t_ui('consoleAbilityPokemonObject'));
        return "";
    }
