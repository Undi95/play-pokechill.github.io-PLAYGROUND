...
shop.shopepochFeather = {
    ...
    name: t_ui('shopNameEpochFeatherX25'),
    price: 10,
...
shop.shopancientOrchid = {
    ...
    name: t_ui('shopNameAncientOrchidX25'),
    price: 10,
...
shop.shopancientKeystone = {
    ...
    name: t_ui('shopNameAncientKeystoneX25'),
    price: 10,
...
shop.shopaetherKeycard = {
    ...
    name: t_ui('shopNameAetherKeycardX25'),
    price: 10,
...
shop.shopfutureDisk = {
    ...
    name: t_ui('shopNameFutureDiskX25'),
    price: 10,
...
shop.shopwisdomPetal = {
    ...
    name: t_ui('shopNameWisdomPetalX25'),
    price: 10,
...
shop.shopbarbaracite = {
    ...
    name: t_ui('shopNameBarbaraciteX21'),
    price: 150,
    currency: item.goldenBottleCap.id,
    category: `evolution`,
...
shop.shophawluchanite = {
    ...
    name: t_ui('shopNameHawluchaniteX21'),
    price: 150,
    currency: item.goldenBottleCap.id,
    category: `evolution`,
...
shop.shoppyroarite = {
    ...
    name: t_ui('shopNamePyroariteX21'),
    price: 150,
    currency: item.goldenBottleCap.id,
    category: `evolution`,
...
shop.shopfalinksite = {
    ...
    name: t_ui('shopNameFalinksiteX21'),
    price: 150,
    currency: item.goldenBottleCap.id,
    category: `evolution`,
...
shop.eventhalloweenTheme = {
    ...
    name: t_ui('shopNamePermanentSeasonalTheme'),
    price: 50,
    stock: 1,
...
ingredient.moomooCheese = {
    renameFR: `Fromage Moomoo`,
    ability: ability.unaware.id,
    price: 20
}

ingredient.fruitBunch = {
    renameFR: `Bouquet de Fruits`,
...
function updateItemShop(){
    ...
    ${t_ui('shopDecorRotation')} <font style="margin-left:0.3rem" class="time-counter-event">...</font>`
    ...
    decorTimer.innerHTML =`
    <svg style="scale:0.7" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12,1A11,11,0,1,0,23,12,11,11,0,0,0,12,1Zm0,20a9,9,0,1,1,9-9A9,9,0,0,1,12,21Z"/><rect width="2" height="7" x="11" y="6" fill="currentColor" rx="1"><animateTransform attributeName="transform" dur="450s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></rect><rect width="2" height="9" x="11" y="11" fill="currentColor" rx="1"><animateTransform attributeName="transform" dur="37.5s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></rect></svg>
    ${t_ui('shopApricornRotation')} <font style="margin-left:0.3rem" class="time-counter-daily">...</font>`
    document.getElementById("shop-listing").appendChild(decorTimer);
    }
    ...
    let stockTag = ""
    if (shop[i].stock) stockTag = ` (${t_ui('shopStock').replace('{stock}', shop[i].stock)})`
    ...
    <img src="img/items/tm${capitalize(move[item[shopItem].move].type)}.png">
...
div.addEventListener("click", () => {
    ...
    document.getElementById("tooltipTop").style.display = "none"
    document.getElementById("tooltipTitle").innerHTML = t_ui('howManyWillYouBuy')
    ...
    function buyItem(amount) {
        ...
        if ( ( item[currency].got>=(shop[i].price*amount) && shop[i].stock == undefined ) || ( item[currency].got>=(shop[i].price*amount) && shop[i].stock >= amount ) ){
            ...
            document.getElementById("tooltipTitle").innerHTML = t_ui('cantAfford');
            document.getElementById("tooltipTop").style.display = "none"    
            document.getElementById("tooltipTop").style.display = "none"    
            document.getElementById("tooltipMid").style.display = "none"
            document.getElementById("tooltipBottom").innerHTML = t_ui('cantAffordPurchase') + `<span id="prevent-tooltip-exit"></span>`
        }
    }
...
function updateIngredientShop(){
    ...
    div.innerHTML = `
        <img src="img/items/${i}.png">
            <span>${(window.gameLang === 'fr' && ingredient[i].renameFR) ? ingredient[i].renameFR : format(i)} (${format(ingredient[i].ability)})</span>
        <strong id="shop-currency-${i}">
...
function updateCurry(){
   ...
   if (saved.lastCurryRotation == rotationWildCurrent){
        ...
        document.getElementById(`curry-title`).innerHTML = t_ui('comeBackIn') + ` <span class="time-counter-daily"></span>`
        updateDailyCounters()
        return
    } else {
    ...
    document.getElementById(`curry-title`).innerHTML = t_ui('selectIngredientsCurry').replace('{count}', curryIngredientList.length)
    ...
    document.getElementById(`curry-pay`).innerHTML = t_ui('payAndCook').replace('{price}', totalPrice)
...
function makeCurry(){
    ...
    if (curryIngredientList.length==0){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('selectAtLeastOneIngredient')
        openTooltip()
        return
    }
    ...
    if (item.goldenBottleCap.got < totalPrice){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('cantAffordThis')
        openTooltip()
        return
    }
    ...
    setTimeout(() => {
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('raidAbilitiesGained')
