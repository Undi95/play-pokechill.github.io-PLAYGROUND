...
function translateItemSubtitle(sub) {
    if (typeof window !== 'undefined' && window.gameLang === 'fr') {
        const map = {
            '(Fighting)': '(Combat)', '(Dark)': '(Ténèbres)', '(Fire)': '(Feu)', '(Dragon)': '(Dragon)',
            '(Fairy)': '(Fée)', '(Rock)': '(Roche)', '(Electric)': '(Électrik)', '(Steel)': '(Acier)',
            '(Grass)': '(Plante)', '(Water)': '(Eau)', '(Ice)': '(Glace)', '(Poison)': '(Poison)',
            '(Flying)': '(Vol)', '(Normal)': '(Normal)', '(Bug)': '(Insecte)', '(Ground)': '(Sol)',
            '(Ghost)': '(Spectre)', '(Psychic)': '(Psy)', '(Hp)': '(PV)', '(Attack)': '(Attaque)',
            '(Defense)': '(Défense)', '(Special Attack)': '(Atq. Spé.)', '(Special Defense)': '(Déf. Spé.)',
            '(Speed)': '(Vitesse)'
        };
        return map[sub] || sub;
    }
...
function format(input) {
    ...
    const isFR = typeof window !== 'undefined' && window.gameLang === 'fr';

    if (isFR && TYPE_NAMES[input]) {
        return TYPE_NAMES[input];
    }

    if (isFR && NATURE_NAMES_FR[input]) {
        return NATURE_NAMES_FR[input];
    }

    if (move[input]) {
        str = isFR && move[input].renameFR ? String(move[input].renameFR) : (move[input].rename ? String(move[input].rename) : str);
    }
    if (pkmn[input]) {
        str = isFR && pkmn[input].renameFR ? String(pkmn[input].renameFR) : (pkmn[input].rename ? String(pkmn[input].rename) : str);
    }
    if (ability[input]) {
        str = isFR && ability[input].renameFR ? String(ability[input].renameFR) : (ability[input].rename ? String(ability[input].rename) : str);
    }
    if (item[input]) {
        if (item[input].type === "tm" && !item[input].rename && item[input].move && move[item[input].move]) {
            const moveId = item[input].move;
            str = isFR && move[moveId].renameFR ? String(move[moveId].renameFR) : (move[moveId].rename ? String(move[moveId].rename) : str);
        } else {
            str = isFR && item[input].renameFR ? String(item[input].renameFR) : (item[input].rename ? String(item[input].rename) : str);
        }
    }
    if (field[input]) {
        str = isFR && field[input].renameFR ? String(field[input].renameFR) : (field[input].rename ? String(field[input].rename) : str);
    }
    if (areas[input]) {
        str = isFR && areas[input].renameFR ? String(areas[input].renameFR) : (areas[input].rename ? String(areas[input].rename) : (areas[input].name ? String(areas[input].name) : str));
    }
...
function setWildPkmn(){
    ...
    document.getElementById(`training-indicator-counter`).textContent = `${t_ui('remaining')} ${currentTrainingWave}`
    ...
    document.getElementById(`spiraling-highest-floor`).innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><defs><mask id="SVGIz3eBe9X"><g fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="4"><path d="M8 44h8m-4 0V4"/><path fill="#555555" d="M40 6H12v16h28l-4-8z"/></g></mask></defs><path fill="currentColor" d="M0 0h48v48H0z" mask="url(#SVGIz3eBe9X)"/></svg>${t_ui('highestFloor')}${saved.maxSpiralFloor}`
    document.getElementById(`spiraling-current-floor`).textContent = `${t_ui('floor')} ${saved.currentSpiralFloor}`
...
for (let t of thresholds) {
    ...
    document.getElementById("explore-wild-name").innerHTML = format(spawnedPkmn) + ` <span class="explore-pkmn-level" > ${t_ui('lvl')} ${wildLevel} </span>`
...
function updateItemsGot(){
    ...
    for (const i in item) {
        ...
        if (item[i].type == "tm") divItem.innerHTML = `<img src="img/items/tm${capitalize(move[item[i].move].type)}.png"> <span>x${item[i].newItem}</span>`;
...
function leaveCombat(){
    ...
    document.getElementById("area-end-item-title").innerHTML = t_ui('newItems')
    ...
    if (saved.currentArea == areas.frontierSpiralingTower.id){ //text
        ...
        if (saved.spiralRewardsClaimed >= 50) {
            setTimeout(() => {
                div.innerHTML = `${t_ui("noMoreRewards")}`
    ...
    if (saved.currentArea == areas.frontierBattleFactory.id){
        ...
        if (battleFactoryScore < minimumScore){
            setTimeout(() => {
                div.innerHTML = t_ui('reachMinimumScore').replace('{score}', minimumScore)
                document.getElementById("area-end-moves-title").appendChild(div);
                document.getElementById("area-end-moves-title").style.display = "flex"          
            }, 1);
            ...
            setTimeout(() => {
                div2.innerHTML = t_ui('nextRewardAt').replace('{score}', nextRewardThreshold)
    ...
    for (const i in item) {
        ...
        if (item[i].type == "tm") divItem.innerHTML = `<img src="img/items/tm${capitalize(move[item[i].move].type)}.png"><span>+${item[i].newItem}</span>`;
    ...
    if (divTag == `<span>${t_ui('shiny')}</span>`) divPkmn.innerHTML = `<img class="sprite-trim" src="img/pkmn/shiny/${hatchedPkmn}.png">`+divTag;
    ...
    if (divTag == `<span>${t_ui('shiny')}</span>`) divPkmn.innerHTML = `<img class="sprite-trim" src="img/pkmn/shiny/${i}.png">`+divTag;
    document.getElementById("area-end-pkmn-list").appendChild(divPkmn);
    ...
    for (i in pkmn) {//new moves notification
        ...
        const div = document.createElement("span");
        div.innerHTML = `${format(i)} ${t_ui('hasLearnt').replace('{moves}', joinWithAnd(pkmn[i].newMoves))}`
    ...
    for (i in pkmn) {//new pokemon notification
        ...
        const div = document.createElement("span");
        div.innerHTML = `${format(i)} ${t_ui('hasBeenUnlocked')}`
    ...
    if (noItems && noPkmn) {
        document.getElementById("area-end-item-title").innerHTML = t_ui('noNewItemsOrPokemon')
        if (saved.currentArea == areas.training.id) document.getElementById("area-end-item-title").innerHTML = `<div style="display:flex; flex-direction:column; width: 100%; height:auto; justify-content:center; align-items:center; flex-shrink:0; text-align:center">${t_ui('trainingFailed')}<br><br>${t_ui('tryToImprovePokemon')}<br> <div class="genetics-overview-tags" > <div style="filter:hue-rotate(0deg)">${t_ui('getBetterTypeMatchingMoves')}</div> <div style="filter:hue-rotate(0deg)">${t_ui('betterStatMoves')}</div> <div style="filter:hue-rotate(0deg)">${t_ui('tipStatUpMoves')}</div> <div style="filter:hue-rotate(0deg)">${t_ui('unlockHiddenAbility')}</div> <div style="filter:hue-rotate(0deg)">${t_ui('getBetterAbility')}</div> <div style="filter:hue-rotate(0deg)">${t_ui('modifyStatsWithGenetics')}</div> </div></div>`
        document.getElementById("area-end-item-title").style.display = "flex"
    }

    //if (areas[saved.currentArea]?.trainer && areas[saved.currentArea]?.defeated) document.getElementById("area-end-item-title").innerHTML = t_ui('rewardsAutoAdded')
    ...
    if ( item.autoRefightTicket.got>0 && areas[saved.currentArea].type!="vs" && areas[saved.currentArea].type!="frontier" && areas[saved.currentArea].id != "training" && areas[saved.currentArea].encounter != true ) {
        ...
        ${t_ui('autoRefight')} <span> ${t_ui('requiresAn').replace('{item}', '<img src="img/items/autoRefightTicket.png"> ' + t_ui('autoRefightTicket'))}</span>
        `
    ...
    if ( areas[saved.currentArea].encounter && areas[saved.currentArea].unlockRequirement && areas[saved.currentArea].unlockRequirement()) {
        ...
        document.getElementById("area-refight").innerHTML = `
        <svg style="margin-right:0.3rem" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 14 14"><path fill="currentColor" fill-rule="evenodd" d="M10.797 2.482a.61.61 0 0 1 0 .866L9.44 4.705h.924c1.393 0 2.305.572 2.845 1.343c.515.736.651 1.593.651 2.153c0 .561-.136 1.418-.651 2.154c-.54.77-1.452 1.342-2.845 1.342c-.948 0-1.695-.48-2.295-1.08c-.584-.584-1.093-1.347-1.56-2.046l-.019-.03c-.49-.734-.936-1.4-1.425-1.889c-.481-.48-.935-.721-1.429-.721c-1.01 0-1.54.39-1.84.82c-.327.466-.43 1.05-.43 1.45s.103.985.43 1.451c.3.43.83.82 1.84.82c.512 0 .982-.259 1.482-.775a.613.613 0 0 1 .88.852c-.612.632-1.379 1.148-2.362 1.148c-1.393 0-2.305-.571-2.845-1.342C.276 9.619.14 8.762.14 8.2s.137-1.418.651-2.153c.54-.77 1.452-1.343 2.845-1.343c.948 0 1.695.48 2.295 1.08c.584.585 1.093 1.348 1.56 2.047l.019.03c.49.734.936 1.4 1.425 1.889c.481.48.935.721 1.429.721c1.01 0 1.54-.39 1.84-.82c.327-.466.43-1.05.43-1.45s-.103-.986-.43-1.451c-.3-.43-.83-.82-1.84-.82H7.961a.613.613 0 0 1-.433-1.046L9.93 2.482a.61.61 0 0 1 .866 0" clip-rule="evenodd"/></svg>
        ${t_ui('autoRefight')} <span> ${t_ui('wontUse').replace('{item}', '<img src="img/items/autoRefightTicket.png"> ' + t_ui('autoRefightTickets'))}</span>
        `
    } 
...
function openMenu(){
    ...
    if (saved.firstTimePlaying){
        ...
        if (esBrave) alert(t_ui('disableBraveShield'));
        });
        }
    ...
    if (typeof mysteryGift !== 'undefined' && today > mysteryGift.duration) document.getElementById(`menu-mystery-gift`).style.display = "none"
...
for (const i in team) {
    ...
    if (pkmn[ team[i].pkmn.id ].exp>=100){ // on level up
        ...
        document.getElementById(`explore-${i}-lvl`).innerHTML = `${t_ui('lvl')} ${pkmn[ team[i].pkmn.id ].level}`
        updateTeamExp()
...
function returnItemLevel(id, mod) {
    ...
    if (mod == "left") {
        if (level === 5) return t_ui('maxLevelReached');
        ...
        return t_ui('leftForNextLevel').replace('{left}', left);
    }
...
function exploreCombatPlayer() {
    ...
    if (barProgressPlayer < 99) {
        ...
        if (nextMove.id == move.metronome.id) {
            ...
            console.info(t_ui('metronomeCasts')+format(selectedMove))
        ...
        for (const member in team) {
            ...
            * 1;
            ...
            if (zUser.type.includes(zTyping)) console.info(t_ui('zMoveStabMultiplier'))
            if (zSplit == `random`) console.info(t_ui('zMoveRandomSplitBonus'))
        ...
        if (saved.currentArea == areas.frontierBattleFactory.id) {
            ...
            document.getElementById(`factory-current-score`).innerHTML = t_ui('scoreLabel').replace('{score}', battleFactoryScore)
        }
...
function initialiseArea(){
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3c4.97 0 9 4.03 9 9"><animateTransform attributeName="transform" dur="1.5s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></path></svg>
           ...
           <div> ${t_ui('autoRefightActive')} <span>${t_ui('autoRefightTicketsRemaining').replace('{count}', item.autoRefightTicket.got).replace('{icon}', '<img src="img/items/autoRefightTicket.png">')}</span> ${t_ui('clickToDisableIt')}</div>
    `
    }
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3c4.97 0 9 4.03 9 9"><animateTransform attributeName="transform" dur="1.5s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></path></svg>
           ...
           <div> ${t_ui('autoRefightActive')} <span>${t_ui('wontConsumeAn').replace('{item}', '<img src="img/items/autoRefightTicket.png"> ' + t_ui('autoRefightTicket'))}</span> ${t_ui('clickToDisableIt')}</div>
    `
    }
...
function setWildAreas() {
    ...
    ${t_ui('rotationHeader')} ${currentWildRotation}/${rotationWildMax}</strong>
    ...
    let seasonTimer = t_ui('limitedAreaUntil').replace('{date}', season[saved.currentSeason].end.month + '/' + season[saved.currentSeason].end.day)
    if (saved.temporalSeason != undefined) seasonTimer = t_ui('limitedAreaUntil').replace('{date}', saved.temporalSeason.end.month + '/' + saved.temporalSeason.end.day)
    ...
    divPark.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span><strong style="background:#B18451">${t_ui('levelAbbreviation')} ${Math.max(1,areas.wildlifePark.level-10)}-${areas.wildlifePark.level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
    ...
    for (const i in areas) {
       ...
       <span>${typeof areas[i].unlockDescription === 'function' ? areas[i].unlockDescription() : areas[i].unlockDescription}</span>
       </span>`
        ticketIndex++
    ...
    if (shinyMark==true) completedFlair = `<svg style="color: rgb(209, 130, 11); opacity: 0.7" class="completed-flair" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15"><path fill="currentColor" d="M14 1v1.5c-.75 0-.75 1.5 0 1.5v1.25c-.75 0-.75 1.5 0 1.5v1.5c-.75 0-.75 1.5 0 1.5V11c-.75 0-.75 1.5 0 1.5V14h-1.5c0-.75-1.5-.75-1.5 0H9.75c0-.75-1.5-.75-1.5 0h-1.5c0-.75-1.5-.75-1.5 0H4c0-.75-1.5-.75-1.5 0H1v-1.5c.75 0 .75-1.5 0-1.5V9.75c.75 0 .75-1.5 0-1.5v-1.5c.75 0 .75-1.5 0-1.5V4c.75 0 .75-1.5 0-1.5V1h1.5c0 .75 1.5.75 1.5 0h1.25c0 .75 1.5.75 1.5 0h1.5c0 .75 1.5.75 1.5 0H11c0 .75 1.5.75 1.5 0zm-2 2H3v9h9zm-1 1v7H4V4z"/></svg>`
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span><strong style="background:#B18451">${t_ui('levelAbbreviation')} ${Math.max(1,areas[i].level-10)}-${areas[i].level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
...
function setDungeonAreas() {
    ...
    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    ${t_ui('rotationHeader')} ${rotationDungeonCurrent}/${rotationDungeonMax}</strong>
    <div class="time-counter-daily"></div>
    </div>
       ...
       <span>${typeof areas[i].unlockDescription === 'function' ? areas[i].unlockDescription() : areas[i].unlockDescription}</span>
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span style="font-size:1.2rem">${format(i)}</span>
                        <span><strong style="background:#6BBC77">${t_ui('levelAbbreviation')} ${Math.max(1,areas[i].level-10)}-${areas[i].level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
                    <span class="explore-ticket-bg" style="background-image: url(img/bg/${areas[i].background}.png);"></span>
...
function setEventAreas() {
    ...
    if (saved.tutorialStep !== "none") {
        ...
        document.getElementById("tooltipMid").innerHTML = t_ui('completeTutorialForEvents')
        openTooltip()
        return
    }
    ...
    if (eventCategory==2 && areas.vsTeamLeaderGiovanni.defeated!=true){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('defeatGiovanniToUnlock')
        openTooltip()
        return
    }
    ...
    if (eventCategory==3){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('notYetImplemented')
        openTooltip()
        return
    }
    ...
    if (rotationEventCurrent==1) eventTitle = t_ui('eventReturnToKanto')
    if (rotationEventCurrent==2) eventTitle = t_ui('eventPrimevalWilds')
    if (rotationEventCurrent==3) eventTitle = t_ui('eventAncientsAwoken')
    if (rotationEventCurrent==4) eventTitle = t_ui('eventAetherTakeover')
    if (rotationEventCurrent==5) eventTitle = t_ui('eventScienceFuture')
    if (rotationEventCurrent==6) eventTitle = t_ui('eventSinnohExpedition')
    ...
    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    ${t_ui('rotationHeader')} ${rotationEventCurrent}/${rotationEventMax}</strong>
  ...
  for (const i in areas) {
       ...
       if (areas[i].encounter && areas[i].difficulty===tier2difficulty && areas.vsEliteFourLance.defeated!=true) unlockRequirement =`<span class="ticket-unlock">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M2 16c0-2.828 0-4.243.879-5.121C3.757 10 5.172 10 8 10h8c2.828 0 4.243 0 5.121.879C22 11.757 22 13.172 22 16s0 4.243-.879 5.121C20.243 22 18.828 22 16 22H8c-2.828 0-4.243 0-5.121-.879C2 20.243 2 18.828 2 16" opacity="0.5"/><path fill="currentColor" d="M6.75 8a5.25 5.25 0 0 1 10.5 0v2.004c.567.005 1.064.018 1.5.05V8a6.75 6.75 0 0 0-13.5 0v2.055a24 24 0 0 1 1.5-.051z"/></svg>
       ${t_ui('defeatEliteFourLance')}</span>`
       ...
       if (areas[i].encounter && areas[i].difficulty===tier4difficulty && areas.vsUltraEntityLusamine.defeated!=true) unlockRequirement =`<span class="ticket-unlock">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M2 16c0-2.828 0-4.243.879-5.121C3.757 10 5.172 10 8 10h8c2.828 0 4.243 0 5.121.879C22 11.757 22 13.172 22 16s0 4.243-.879 5.121C20.243 22 18.828 22 16 22H8c-2.828 0-4.243 0-5.121-.879C2 20.243 2 18.828 2 16" opacity="0.5"/><path fill="currentColor" d="M6.75 8a5.25 5.25 0 0 1 10.5 0v2.004c.567.005 1.064.018 1.5.05V8a6.75 6.75 0 0 0-13.5 0v2.055a24 24 0 0 1 1.5-.051z"/></svg>
       ${t_ui('defeatUltraEntityLusamine')}</span>`


       let eventTag ;
       eventTag = `<strong class="event-tag">${t_ui('wildZone')}</strong>`
       if (areas[i].uncatchable) eventTag = `<strong style="filter:hue-rotate(140deg)" class="event-tag">${t_ui('collection')}</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier1difficulty) eventTag = `<strong style="filter:hue-rotate(50deg)" class="event-tag">${t_ui('tierIRaid')}</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier2difficulty) eventTag = `<strong style="filter:hue-rotate(200deg)" class="event-tag">${t_ui('tierIiRaid')}</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier3difficulty) eventTag = `<strong style="filter:hue-rotate(300deg)" class="event-tag">${t_ui('tierIiiRaid')}</strong>`
       if (areas[i].encounter && areas[i].difficulty===tier4difficulty) eventTag = `<strong style="filter:hue-rotate(100deg)" class="event-tag">${t_ui('tierIvRaid')}</strong>`
       ...
       if (areas[i].name) eventName = areas[i].name
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span><strong style="background:#D57392">${t_ui('levelAbbreviation')} ${levelrange}</strong>${eventTag}<span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
                    <span class="explore-ticket-bg" style="background-image: url(img/bg/${areas[i].background}.png);"></span>
...
function updateRaidTimer(){
    ...
    document.getElementById("raid-timer-indicator-current").innerHTML = t_ui('timeLeft').replace('{time}', formattedTime)
...
document.getElementById("pokedex-search").addEventListener("keydown", e => {
  if (e.key === "Enter") {
    ...
    // Normalize French type queries to English IDs when language is French
    if (window.gameLang === 'fr') {
      ...
      searchValue = searchValue.split(/\s+/).map(term => FR_TYPE_TO_EN[term.toLowerCase().trim()] || term).join(' ');
    }
...
function updatePokedex(){
    ...
    if (dexTeamSelect != undefined) {
        ...
        document.getElementById("pokedex-filters-title").innerHTML = t_ui('selectPokemonAddTeam')
    }

    if (tmToTeach != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToTeach').replace('{move}', format(tmToTeach))}`
    }

    if (memoryToTeach != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToTeach').replace('{move}', format(memoryToTeach))}`
    }

    if (evoItemToUse != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToUseItem').replace('{item}', format(evoItemToUse))}`
    }

    if (vitaminToUse != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToUseItem').replace('{item}', format(vitaminToUse))}`
    }

    if (itemToUse != undefined) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToUseItem').replace('{item}', format(itemToUse))}`
    }

    if (dexHostSelect == true) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = t_ui('selectHostPokemon')
    }

    if (dexTrainSelect == true) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = t_ui('selectPokemonToTrain')
    }

    if (dexSampleSelect == true) {
        document.getElementById("pokedex-filters-title").style.display = "flex"
        document.getElementById("pokedex-filters-cancel").style.display = "flex"
        document.getElementById("pokedex-filters-title").innerHTML = t_ui('selectSamplePokemon')
    }
...
fusePkmn = new Fuse(sortedPokemon, {
    keys: [ { name: 'name', getFn: obj => obj.id }, { name: 'renameFR', getFn: obj => obj.renameFR || '' }, 'type', "level", `ability`, `hiddenAbility.id`, `eggMove.id`, `movepool`, `movepoolFR`,'tagShiny','tagShinyFR','tagPokerus','tagPokerusFR', "tagSignature","tagSignatureFR","tagCaughtFR","tagObtainedInFR"],
    threshold: 0.1,
 ...
 // shitty hack, refilter after search
    ...
    for (const p of sortedPokemon) {
        ...
        */
        ...
        div.innerHTML = `<span style="display:flex; white-space:nowrap">${t_ui('lvl')} ${pkmn[i].level} ${nameMarks}</span><img class="sprite-trim" src="img/pkmn/sprite/${i}.png">`
        if (pkmn[i].shiny) div.innerHTML = `<span style="display:flex; white-space:nowrap">${t_ui('lvl')} ${pkmn[i].level} ${nameMarks}</span> <img class="sprite-trim" src="img/pkmn/shiny/${i}.png">`
        if (pkmn[i].shiny && pkmn[i].shinyDisabled == true) div.innerHTML = `<span style="display:flex; white-space:nowrap">${t_ui('lvl')} ${pkmn[i].level} ${nameMarks}</span><img class="sprite-trim" src="img/pkmn/sprite/${i}.png">`


        if (pkmn[i].starsign){
        div.innerHTML = `<span style="display:flex; white-space:nowrap">${t_ui('lvl')} ${pkmn[i].level} ${nameMarks}</span><img style="filter:hue-rotate(${starsign[pkmn[i].starsign].hue}deg)" class="sprite-trim" src="img/pkmn/sprite/${i}.png">`
        if (pkmn[i].shiny) div.innerHTML = `<span style="display:flex; white-space:nowrap">${t_ui('lvl')} ${pkmn[i].level} ${nameMarks}</span> <img style="filter:hue-rotate(${starsign[pkmn[i].starsign].hue}deg)" class="sprite-trim" src="img/pkmn/shiny/${i}.png">`
        if (pkmn[i].shiny && pkmn[i].shinyDisabled == true) div.innerHTML = `<span style="display:flex; white-space:nowrap">${t_ui('lvl')} ${pkmn[i].level} ${nameMarks}</span><img style="filter:hue-rotate(${starsign[pkmn[i].starsign].hue}deg)" class="sprite-trim" src="img/pkmn/sprite/${i}.png">`
        }
        ...
        if (evoItemToUse != undefined ) {
            ...
            div.addEventListener("click", e => { 
                ...
                document.getElementById("tooltipBottom").innerHTML = `${format(pkmn[ pkmn[i].evolve()[evo].pkmn.id ].id)} ${t_ui('hasBeenUnlocked2')}`
                openTooltip()
                updateItemBag()
        ...
        givePkmn(pkmn[ pkmn[i].evolve()[1].pkmn.id ],1)
            ...
            if (itemToUse == item.heartScale.id){
                ...
                document.getElementById("tooltipBottom").style.display = "none" 
                document.getElementById("tooltipTitle").innerHTML = t_ui('selectMoveToRemember')
                document.getElementById("tooltipTop").style.display = "none"    
            ...
            if (itemToUse == item.abilityPatch.id){
                ...
                document.getElementById("tooltipTitle").innerHTML = t_ui('newAbility')
                document.getElementById("tooltipTop").style.display = "none"    
                ...
                if (ability[newAbility].rarity===2) document.getElementById("tooltipMid").innerHTML = `<div class="genetics-overview-tags"><div style="filter:hue-rotate(100deg)">★ ${format(newAbility)} (${t_ui('uncommon')}!)</div></div>`
                if (ability[newAbility].rarity===3) document.getElementById("tooltipMid").innerHTML = `<div class="genetics-overview-tags"><div style="filter:hue-rotate(200deg)">★ ${format(newAbility)} (${t_ui('rareExcl')}!)</div></div>`
...
function tagMenuAssign(){
    ...
    if (!saved.tagSystemTags || saved.tagSystemTags.length === 0) {
        document.getElementById("tag-listing").innerHTML = t_ui('noTagsCreatedShort');
        return;
    }
...
function updateEditorTags(){
    if (pkmn[currentEditedPkmn].tagList?.length > 0) {
        ...
        document.getElementById(`tag-system-editor-tags`).innerHTML = t_ui('clickAddTags') // No tags, show prompt
        return
    }
...
function tagMenuRemove(id){
    document.getElementById("tooltipBottom").style.display = "none"
    document.getElementById("tooltipMid").innerHTML = `<div id="remove-tag-menu" style="display:flex; flex-direction:column">${t_ui('deleteTagConfirm')}</div>`
    const div = document.createElement(`div`)
    div.className = `tag-system-tag`
    ...
    document.getElementById(`remove-tag-menu`).innerHTML+= `<div class="tag-system-delete" onclick="tagMenuRemoveConfirm(${id})">${t_ui('deleteTag')}</div>`
}
...
function tagMenuCreate(){
    ...
    document.getElementById("tooltipMid").innerHTML = `
        ...
        <div class="tag-system-form-section">
            <div class="tag-system-form-group">
                ...
                <input type="text" id="tag-system-tagName" placeholder="Enter tag name..." data-i18n-placeholder="tagNamePlaceholder">
            </div>
    ...
    `
        ...
        function tagSystemSaveTag() {
            ...
            if (!tagName) {
                alert(t_ui('pleaseEnterTagName'));
                return;
            }
 
            if (!tagSystemSelectedIcon) {
                alert(t_ui('pleaseSelectIcon'));
                return;
            }
...
function switchMenu(id){
    ...
    if (/vs|items|team|dex|dictionary|guide/.test(id) && saved.tutorialStep != "none") {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('completeTutorialFirst')
        openTooltip()
        return
    }
    ...
    if (id=="shop" && areas.vsGymLeaderBrock.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('defeatBrockToUnlock')
        openTooltip()
        return
    }
    ...
    if (id=="dimension" && areas.vsLegendTrainerBrendan.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('defeatBrendanToUnlock')
        openTooltip()
        return
    }
    ...
    if (id=="genetics" && areas.vsEliteFourLance.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('defeatLanceToUnlock2')
        openTooltip()
        return
    }
    ...
    if (id=="training" && areas.vsGymLeaderMisty.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('defeatMistyToUnlock')
        openTooltip()
        return
    }
...
function updateItemBag(){
    ...
    for (const i in item) {
        ...
        let subtitle = ""
        if (item[i].subtitle) subtitle = `<text style="opacity:0.6; margin-left:0.2rem">${translateItemSubtitle(item[i].subtitle)}</text>`


        div.dataset.item = i
        if (item[i].type == "tm") div.innerHTML = `<img src="img/items/tm${capitalize(move[item[i].move].type)}.png"> <span class="item-list-name">${format(i)} ${subtitle}<strong style="opacity:0.6; font-weight:200; white-space:nowrap; font-size:0.9rem; margin-left:0.2rem"> (${move[item[i].move].power} BP, ${format(move[item[i].move].split).slice(0, 3)})</strong> </span>  <span>x${item[i].got}</span>`
        ...
        if (item[i].type == "tm" && dexTeamSelect==undefined) {
            ...
            document.getElementById("pokedex-filters-title").style.display = "flex"
            document.getElementById("pokedex-filters-cancel").style.display = "flex"
            document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToTeach').replace('{move}', format(item[i].move))}`
            document.getElementById("menu-button-parent").style.display = "none"
        ...
        if (item[i].type == "memory" && dexTeamSelect==undefined) {
            ...
            document.getElementById("pokedex-filters-title").style.display = "flex"
            document.getElementById("pokedex-filters-cancel").style.display = "flex"
            document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToTeach').replace('{move}', format(item[i].ability))}`
            document.getElementById("menu-button-parent").style.display = "none"
            document.getElementById(`item-menu`).style.display = "none"
        ...
        if (item[i].evo && dexTeamSelect==undefined) {
            ...
            document.getElementById("pokedex-filters-title").style.display = "flex"
            document.getElementById("pokedex-filters-cancel").style.display = "flex"
            document.getElementById("pokedex-filters-title").innerHTML = `${t_ui('selectPokemonToUseItem').replace('{item}', format(i))}`
            document.getElementById("menu-button-parent").style.display = "none"
            document.getElementById(`item-menu`).style.display = "none"
...
function updateVS() {
 ...
 for (const i in areas) {
    ...
    if (areas[i].defeated) continue;
       ...
       let fieldTag = ""
       if (areas[i].fieldEffect) fieldTag = `<strong style="font-size:1rem; background:#725AA4; margin-left:0.2rem">+ ${t_ui('fieldEffect')}</strong>`
        ...
        divAreas.innerHTML = `
                ...
                <div class="vs-card-bg"></div>
                    <span class="explore-ticket-left" style="z-index: 2;">
                        <span id="trainer-name-${areas[i].name}" style="font-size:1.3rem">${format(i)}${nameTag}</span>
                        <span><strong style="font-size:1rem; background:#465f96">${t_ui('levelAbbreviation')} ${Math.max(1,(areas[i].level))}</strong>${fieldTag}</span>
                    </span>
                <div>
                </div>
 ...
 if (document.getElementById(`vs-listing`).innerHTML == "") document.getElementById(`vs-listing`).innerHTML = `<div style="display:flex; flex-direction:column; justify-content:center; align-items:center; background:#ECDEB7; border-radius:0.3rem; height:15rem; width:15rem; text-align:center"><img src="img/pkmn/sprite/pikachuRockstar.png">${t_ui('allTrainersDefeated')}<br><span style="font-size:0.9rem; opacity:0.7">${t_ui('howAboutBattleFrontier')}</span></div>`
...
function updateFrontier() {
    ...
    let divisionText = ``
    if (rotationFrontierCurrent==1) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}${t_ui('littleCup')}${cupsvg}</span><div>${returnDivisionLetter("C")} ${t_ui('divisionAndBelow')}</div>`
    if (rotationFrontierCurrent==2) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}${t_ui('greatLeague')}${cupsvg}</span><div>${returnDivisionLetter("B")} ${t_ui('divisionAndBelow')}</div>`
    if (rotationFrontierCurrent==3) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}${t_ui('ultraLeague')}${cupsvg}</span><div>${returnDivisionLetter("A")} ${t_ui('divisionAndBelow')}</div>`
    if (rotationFrontierCurrent==4) divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}${t_ui('masterLeague')}${cupsvg}</span><div>${returnDivisionLetter("SSS")} ${t_ui('divisionAndBelow')}</div>`
    ...
    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    ${t_ui('rotationLabel').replace('{current}', rotationFrontierCurrent).replace('{max}', rotationFrontierMax)}</strong>
    <div class="time-counter-event"></div>
    </div>

    <div class="rotation-timer">
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    ${t_ui('trainerReset')}</strong>
    <div class="time-counter-daily"></div>
    </div>
    </div>
    ...
    spiral.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">${t_ui('battleTower')}</span>
        <span><strong style="background:#964646ff">${t_ui('highestReachedFloorLabel')}${saved.maxSpiralFloor}</strong><span></span></span>
        </span>
        </div>
        <div style="width: 8rem;" class="explore-ticket-right">
    ...
    factory.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">${t_ui('battleFactory')}</span>
        <span><strong style="background:#964646ff">${t_ui('highestScore')}${saved.maxFactoryScore}</strong><span></span></span>
        </span>
        </div>
        <div style="width: 8rem;" class="explore-ticket-right">
    ...
    arena.innerHTML = `
        ...
        <span class="explore-ticket-left">
        <span style="font-size:1.2rem">${t_ui('battleArena')}</span>
        <span><strong style="background:#964646ff">${t_ui('defeatedLabel')}${saved.arenaCurrentTrainer-1}/5</strong><span></span></span>
        </span>
        </div>
        <div style="width: 8rem;" class="explore-ticket-right">
...
function updateWildBuffs(){
    ...
    if (saved.weather!==undefined && saved.weatherTimer>0){
        ...
        div.innerHTML = `${t_ui('tag' + capitalize(saved.weather))} ${saved.weatherTimer} ${weatherIcon}`;
        document.getElementById("wild-buff-list").appendChild(div);
    }
...
const tagBurn = `<span data-buff="burn"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fire")}">${t_ui('tagBurn')}</span></span>`
const tagFreeze = `<span data-buff="freeze"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ice")}">${t_ui('tagFreeze')}</span></span>`
const tagParalysis = `<span data-buff="paralysis"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("electric")}">${t_ui('tagParalysis')}</span></span>`
const tagPoisoned = `<span data-buff="poisoned"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("poison")}">${t_ui('tagPoisoned')}</span></span>`
const tagSleep = `<span data-buff="sleep"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("normal")}">${t_ui('tagSleep')}</span></span>`
const tagConfused = `<span data-buff="confused"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("psychic")}">${t_ui('tagConfused')}</span></span>`

const tagEmbargo = `<span data-buff="embargo"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("dark")}">${t_ui('tagEmbargo')}</span></span>`

const tagSunny = `<span data-buff="sunny"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fire")}">${t_ui('tagSunny')}</span></span>`
const tagRainy = `<span data-buff="rainy"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("water")}">${t_ui('tagRainy')}</span></span>`
const tagSandstorm = `<span data-buff="sandstorm"><span style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ground")}">${t_ui('tagSandstorm')}</span></span>`
const tagHail = `<span data-buff="hail"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ice")}">${t_ui('tagHail')}</span></span>`
const tagFoggy = `<span data-buff="foggy"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ghost")}">${t_ui('tagFoggy')}</span></span>`
const tagElectricTerrain = `<span data-buff="electricTerrain"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("electric")}">${t_ui('tagElectricTerrain')}</span></span>`
const tagGrassyTerrain = `<span data-buff="grassyTerrain"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("grass")}">${t_ui('tagGrassyTerrain')}</span></span>`
const tagMistyTerrain = `<span data-buff="mistyTerrain"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fairy")}">${t_ui('tagMistyTerrain')}</span></span>`


const tagTrickRoom = `<span data-buff="trickRoom"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("dark")}">${t_ui('tagTrickRoom')}</span></span>`
const tagWeirdRoom = `<span data-buff="weirdRoom"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("bug")}">${t_ui('tagWeirdRoom')}</span></span>`
const tagCrossRoom = `<span data-buff="crossRoom"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fighting")}">${t_ui('tagCrossRoom')}</span></span>`
...
const tagLightScreen = `<span data-buff="lightScreen"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("fairy")}">${t_ui('tagLightScreen')}</span></span>`
const tagSafeguard = `<span data-buff="safeguard"><span  style="color:white;cursor:help;padding: 0.1rem 0.7rem; border-radius: 0.2rem; font-size:1.1rem; width: auto; background: ${returnTypeColor("ice")}">${t_ui('tagSafeguard')}</span></span>`
...
function formatBuffs(buff,mod) {
    ...
    if (mod==undefined){
    if (buff==="embargo") return t_ui('buffEmb')
    if (buff==="burn") return t_ui('buffBrn')
    if (buff==="freeze") return t_ui('buffFrz')
    if (buff==="paralysis") return t_ui('buffPar')
    if (buff==="poisoned") return t_ui('buffPsn')
    if (buff==="sleep") return t_ui('buffZzz')
    if (buff==="confused") return t_ui('buffCnf')
    if (buff==="atkup1") return t_ui('buffAtk') + ' ▲'
    if (buff==="atkup2") return t_ui('buffAtk') + ' ▲▲'
    if (buff==="atkdown1") return t_ui('buffAtk') + ' ▼'
    if (buff==="atkdown2") return t_ui('buffAtk') + ' ▼▼'
    if (buff==="defup1") return t_ui('buffDef') + ' ▲'
    if (buff==="defup2") return t_ui('buffDef') + ' ▲▲'
    if (buff==="defdown1") return t_ui('buffDef') + ' ▼'
    if (buff==="defdown2") return t_ui('buffDef') + ' ▼▼'
    if (buff==="satkup1") return t_ui('buffSatk') + ' ▲'
    if (buff==="satkup2") return t_ui('buffSatk') + ' ▲▲'
    if (buff==="satkdown1") return t_ui('buffSatk') + ' ▼'
    if (buff==="satkdown2") return t_ui('buffSatk') + ' ▼▼'
    if (buff==="sdefup1") return t_ui('buffSdef') + ' ▲'
    if (buff==="sdefup2") return t_ui('buffSdef') + ' ▲▲'
    if (buff==="sdefdown1") return t_ui('buffSdef') + ' ▼'
    if (buff==="sdefdown2") return t_ui('buffSdef') + ' ▼▼'
    if (buff==="speup1") return t_ui('buffSpe') + ' ▲'
    if (buff==="speup2") return t_ui('buffSpe') + ' ▲▲'
    if (buff==="spedown1") return t_ui('buffSpe') + ' ▼'
    if (buff==="spedown2") return t_ui('buffSpe') + ' ▼▼'
    }
...
function claimExportReward(){
        ...
        document.getElementById("tooltipTitle").innerHTML = t_ui('rewardReceived')
        document.getElementById("tooltipMid").style.display = `none`
...
let geneticItemSelect = false
    ...
    document.getElementById("genetics-start").addEventListener("click", e => {
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = t_ui('operationFinished')
        document.getElementById("tooltipMid").innerHTML = t_ui('itemWillBeConsumed')
        ...
        setGeneticMenu("end");
        '
        style="cursor:pointer; font-size:2rem; width:40%"> ${t_ui('nope')} </div>
        ...
        ' style="cursor:pointer; font-size:2rem; width:40%" id="prevent-tooltip-exit">${t_ui('yeah')}
        </div>
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = t_ui('abortOperationConfirm')
        document.getElementById("tooltipMid").innerHTML = t_ui('nothingButTimeLost')
        ...
        " style="cursor:pointer; font-size:2rem" id="prevent-tooltip-exit">${t_ui('yeah')}</div>`
        openTooltip()
...
document.getElementById("genetics-compat-text").innerHTML = `${t_ui('compatibility')} <font style="color:#E58FFF; margin-left:0.3rem">[${compability-1}]</font>`
...
document.getElementById("genetics-data-time").innerHTML = t_ui('timeHMS').replace('{h}', h).replace('{m}', m).replace('{s}', x)
...
if (saved.geneticOperation !== undefined){
    document.getElementById("genetics-start").textContent = t_ui('abort')
    ...
    document.getElementById("genetics-start").textContent = t_ui('start')
...
if (saved.geneticOperation <= 1){
    document.getElementById("genetics-start").textContent = t_ui('finish')
...
if (mod==="end"){
    ...
    if (rng(shinyChance)) {pkmn[saved.geneticHost].shiny = true; summaryTags += `<div style="filter:hue-rotate(100deg)">${t_ui('shinyMutation')}</div>` }
    ...
    summaryTags += `<div style="filter:hue-rotate(-50deg)">${t_ui('abilitySwapped')}</div>`
    } else {
    const newAbility = learnPkmnAbility(saved.geneticHost,10) //boosted chance
    if (itemUsed=="everstone") {pkmn[saved.geneticHost].ability = newAbility; summaryTags += `<div style="filter:hue-rotate(-50deg)">${t_ui('newAbility').replace('{ability}', format(newAbility))}</div>`}
    }
    ...
    pkmn[saved.geneticSample].movepool.forEach(moveID => {
        if (rng(moveChance) && !(pkmn[saved.geneticHost].movepool.includes(moveID)) && (move[moveID].moveset!==undefined ||  (   move[moveID].moveset==undefined && pkmn[saved.geneticHost].eggMove?.id == moveID  )    ||   ( move[moveID].moveset==undefined && ["B", "C", "D"].includes(returnPkmnDivision(pkmn[saved.geneticHost])) && item.replicatorUpgradeE.got>0 && compability>=3   )   ) ) {
            ...
            if (move[moveID].moveset==undefined && (pkmn[saved.geneticSample].signature?.id == moveID || pkmn[saved.geneticSample].eggMove?.id == moveID)  ) summaryTags += `<div style="filter:hue-rotate(200deg)">${t_ui('eggMoveInherited').replace('{move}', format(moveID))}</div>`
            else summaryTags += `<div style="filter:hue-rotate(0deg)">${t_ui('moveInherited').replace('{move}', format(moveID))}</div>`
        }
    });
    ...
    if (itemUsed==`lockCapsule`){
        ...
        summaryTags += `<div style="filter:hue-rotate(-200deg)">${t_ui('movesTransferred')}</div>`
    ...
    if (rng(ivChanceHp) && pkmn[saved.geneticHost].ivs.hp<Math.min(ivCap, pkmn[saved.geneticSample].ivs.hp)) {pkmn[saved.geneticHost].ivs.hp = Math.min(ivCap, pkmn[saved.geneticSample].ivs.hp) ; summaryTags += `<div style="filter:hue-rotate(200deg)">${t_ui('hpIvInherited')}</div>`}
    if (rng(ivChanceAtk) && pkmn[saved.geneticHost].ivs.atk<Math.min(ivCap, pkmn[saved.geneticSample].ivs.atk)) {pkmn[saved.geneticHost].ivs.atk = Math.min(ivCap, pkmn[saved.geneticSample].ivs.atk) ; summaryTags += `<div style="filter:hue-rotate(200deg)">${t_ui('atkIvInherited')}</div>`}
    if (rng(ivChanceDef) && pkmn[saved.geneticHost].ivs.def<Math.min(ivCap, pkmn[saved.geneticSample].ivs.def)) {pkmn[saved.geneticHost].ivs.def = Math.min(ivCap, pkmn[saved.geneticSample].ivs.def) ; summaryTags += `<div style="filter:hue-rotate(200deg)">${t_ui('defIvInherited')}</div>`}
    if (rng(ivChanceSatk) && pkmn[saved.geneticHost].ivs.satk<Math.min(ivCap, pkmn[saved.geneticSample].ivs.satk)) {pkmn[saved.geneticHost].ivs.satk = Math.min(ivCap, pkmn[saved.geneticSample].ivs.satk) ; summaryTags += `<div style="filter:hue-rotate(200deg)">${t_ui('satkIvInherited')}</div>`}
    if (rng(ivChanceSdef) && pkmn[saved.geneticHost].ivs.sdef<Math.min(ivCap, pkmn[saved.geneticSample].ivs.sdef)) {pkmn[saved.geneticHost].ivs.sdef = Math.min(ivCap, pkmn[saved.geneticSample].ivs.sdef) ; summaryTags += `<div style="filter:hue-rotate(200deg)">${t_ui('sdefIvInherited')}</div>`}
    if (rng(ivChanceSpe) && pkmn[saved.geneticHost].ivs.spe<Math.min(ivCap, pkmn[saved.geneticSample].ivs.spe)) {pkmn[saved.geneticHost].ivs.spe = Math.min(ivCap, pkmn[saved.geneticSample].ivs.spe) ; summaryTags += `<div style="filter:hue-rotate(200deg)">${t_ui('speIvInherited')}</div>`}
    ...
    for (const iv in pkmn[saved.geneticHost].ivs){
        ...
        if (newIv>ivId) {
            ...
            if (iv === "hp") summaryTags += `<div style="filter:hue-rotate(250deg)">${t_ui('hpIvIncreased')}</div>`
            if (iv === "atk") summaryTags += `<div style="filter:hue-rotate(250deg)">${t_ui('atkIvIncreased')}</div>`
            if (iv === "def") summaryTags += `<div style="filter:hue-rotate(250deg)">${t_ui('defIvIncreased')}</div>`
            if (iv === "satk") summaryTags += `<div style="filter:hue-rotate(250deg)">${t_ui('satkIvIncreased')}</div>`
            if (iv === "sdef") summaryTags += `<div style="filter:hue-rotate(250deg)">${t_ui('sdefIvIncreased')}</div>`
            if (iv === "spe") summaryTags += `<div style="filter:hue-rotate(250deg)">${t_ui('speIvIncreased')}</div>`
        }
    }


    if (summaryTags == "") summaryTags = t_ui('noNewGeneticChanges')
    
    document.getElementById("tooltipTitle").innerHTML = t_ui('operationOverview')
    document.getElementById("tooltipTop").style.display = "none"    
...
setInterval(() => {
    ...
    if (saved.geneticOperation<1 && document.getElementById("genetics-start").textContent == t_ui('abort')) setGeneticMenu();
...
training.level = {
    name: t_ui('trainingLevelName'),
    info: t_ui('trainingLevelInfo'),
    tier: 2,
    color: `#dfc969`,
    errorText: t_ui('trainingLevelError'),
    ...
    effect: function() {
        ...
        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ${t_ui('isNowLevel')} ${pkmn[saved.trainingPokemon].level}!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
...
training.iv1 = { //disapears if you have more than x ivs
    name: t_ui('trainingIv1Name'),
    info: t_ui('trainingIv1Info'),
    tier: 1,
    color: `#699edf`,
    condition: function() {
    ...
    errorText: t_ui('trainingIv1Error'),
    effect: function() {
    ...
    const statNames = {hp: t_ui('statHp'), atk: t_ui('statAtk'), def: t_ui('statDef'), satk: t_ui('statSatk'), sdef: t_ui('statSdef'), spe: t_ui('statSpe')};
    const parts = Object.entries(increases).map(
        ([stat, n]) => `${statNames[stat]} ${n} ${n > 1 ? t_ui('points') : t_ui('point')}`
    );

    let text;
    if (parts.length === 1) text = t_ui('increasedStat').replace('{stat}', parts[0]);
    else text = t_ui('increasedStats').replace('{rest}', parts.slice(0, -1).join(", ")).replace('{last}', parts.at(-1));
    pkmn[i].dictionaryTagIvSum = pkmn[i].ivs.hp + pkmn[i].ivs.atk + pkmn[i].ivs.satk + pkmn[i].ivs.spe + pkmn[i].ivs.sdef + pkmn[i].ivs.def
...
training.iv2 = { //doesnt appear until you have more than x ivs
    name: t_ui('trainingIv2Name'),
    info: t_ui('trainingIv2Info'),
    tier: 2,
    color: `#699edf`,
    condition: function() {
        const id = pkmn[saved.trainingPokemon]
    ...
    errorText: t_ui('trainingIv2Error'),
    effect: function() {
...
training.iv3 = { //doesnt appear until you have more than x ivs
    name: t_ui('trainingIv3Name'),
    info: t_ui('trainingIv3Info'),
    tier: 3,
    color: `#699edf`,
    condition: function() {
        const id = pkmn[saved.trainingPokemon]
    ...
    errorText: t_ui('trainingIv3Error'),
    effect: function() {
...
training.ability = {
    name: t_ui('trainingAbilityName'),
    info: t_ui('trainingAbilityInfo'),
    tier: 1,
    color: `#69df96`,
    effect: function() {
        ...
        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ${t_ui('nowHas')} ${format(newAbility)}!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
        document.getElementById("area-end-item-title").style.display = "none"
        }, 10);
...
training.hiddenAbility = {
    name: t_ui('trainingHiddenAbilityName'),
    info: t_ui('trainingHiddenAbilityInfo'),
    tier: 2,
    color: `#69df96`,
    ...
    errorText: t_ui('trainingHiddenAbilityError'),
    effect: function() {
        pkmn[saved.trainingPokemon].hiddenAbilityUnlocked = true

        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ${t_ui('nowHas')} ${format(pkmn[saved.trainingPokemon].hiddenAbility.id)}!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
        document.getElementById("area-end-item-title").style.display = "none"
        }, 10);
...
training.move = { //disapears if you have 20+ moves or smth
    name: t_ui('trainingMoveName'),
    info: t_ui('trainingMoveInfo'),
    tier: 1,
    color: `#cf79c1`,
    ...
    errorText: t_ui('trainingMoveError'),
    effect: function() {
        let learntMove = learnPkmnMove(pkmn[saved.trainingPokemon].id, pkmn[saved.trainingPokemon].level)
        ...
        setTimeout(() => {
        const div = document.createElement("span");
        div.innerHTML = `${format(saved.trainingPokemon)} ${t_ui('learnt')} ${format(learntMove)}!`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
        document.getElementById("area-end-item-title").style.display = "none"
        }, 10);
...
training.nature = {
    name: t_ui('trainingNatureName'),
    info: t_ui('trainingNatureInfo'),
    tier: 3,
    color: `#DF7A69`,
    condition: function() { if (areas.vsLegendTrainerBrendan.defeated == true) return true },
    errorText: t_ui('trainingNatureError'),
    effect: function() {
        ...
        setTimeout(() => {
        const div = document.createElement("span");
        const natureLabel = window.gameLang === 'fr' ? `${t_ui('natureSuffix')} ${format(pickedNature)}` : `${format(pickedNature)} ${t_ui('natureSuffix')}`;
        div.innerHTML = `${format(saved.trainingPokemon)} ${t_ui('nowHasNature')} ${natureLabel}`
        document.getElementById("area-end-moves-title").appendChild(div);
        document.getElementById("area-end-moves-title").style.display = "flex"
        document.getElementById("area-end-item-title").style.display = "none"
        }, 10);
...
function setTrainingMenu() {
    ...
    <strong style="outline-color: ${training[i].color}; color: ${training[i].color}" >${t_ui('difficulty')}: ${returnStars(training[i].tier)}</strong>
    `
    ...
    if (restrictedError) {
        document.getElementById("tooltipTop").style.display = "none"
        document.getElementById("tooltipBottom").style.display = "none"
        document.getElementById("tooltipTitle").innerHTML = t_ui('restrictedMoves')
        document.getElementById("tooltipMid").innerHTML = t_ui('trainingPokemonRestrictedMoves').replace('{icon}', restrictedIcon)
        openTooltip()
        return
    }
...
function battleSummary() {
    ...
    document.getElementById("tooltipTop").style.display = `none`
    document.getElementById("tooltipTitle").innerHTML = t_ui('damageDealt')
    document.getElementById("tooltipBottom").style.display = `none`
...
function claimWonderTrade(){
    ...
    if (areas.vsMasterTrainerGeeta.defeated == false) {
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('defeatGeetaToUnlock')
        openTooltip()
        return
    }
    ...
    document.getElementById("tooltipTop").style.display = "none"
    document.getElementById("tooltipTitle").innerHTML = t_ui('wonderTrade')
    document.getElementById("tooltipMid").innerHTML = t_ui('randomPokemonEvery12h')
    document.getElementById("tooltipBottom").innerHTML = `

        <div onclick="wonderTrade()" class="custom-challenge-button" style="margin-top:0.5rem">${t_ui('letsDoIt')}</div>
...
function wonderTrade(){
    ...
    document.getElementById("wonder-text").innerHTML = `${t_ui('thanksForPokemon')} ${format(chosenPokemon)}!`
...
function claimMysteryGift(){
    ...
    document.getElementById("tooltipTitle").innerHTML = t_ui('mysteryGift')
    document.getElementById("tooltipMid").innerHTML = `${mysteryGift.info}<br>${t_ui('mysteryGiftClaimUntil').replace('{date}', mysteryGift.duration.toLocaleString("en-US", {month: "long",day: "numeric"}))}`
...
function pkmnWalk(){
    ...
    if (!pickedPkmn || typeof pickedPkmn !== 'string' || !pkmn[pickedPkmn]) return
...
function seasonalSwitch(){
    ...
    //document.getElementById("tooltipTop").innerHTML = `<img src="img/pkmn/shiny/${mysteryGift.icon}.png">`
    document.getElementById("tooltipTitle").innerHTML = t_ui('selectAnEvent')
    document.getElementById("tooltipMid").innerHTML = t_ui('selectSeasonalEvent')
...
function seasonCheck() {
    ...
    if (saved.halloweenThemeUnlocked == true) document.getElementById('settings-theme').innerHTML += '<option value="spooky">' + t_ui('themeSpooky') + '</option>';


    if (saved.currentSeason !== undefined && saved.temporalSeason == undefined) {
    document.getElementById('shop-categories').innerHTML += `<div onclick="shopCategory = 'limited'; updateItemShop() "><img src="img/items/cherishball.png">${t_ui('limited')}</div>`;
    }
...
function renamePokemon(){
    document.getElementById("tooltipTop").style.display = "none"
    document.getElementById("tooltipBottom").style.display = "none"
    document.getElementById("tooltipTitle").innerHTML = t_ui('giveNickname')
    document.getElementById("tooltipMid").innerHTML = `
...
function updateMegaDimension(tier){
    ...
    <strong><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M6.94 2c.416 0 .753.324.753.724v1.46c.668-.012 1.417-.012 2.26-.012h4.015c.842 0 1.591 0 2.259.013v-1.46c0-.4.337-.725.753-.725s.753.324.753.724V4.25c1.445.111 2.394.384 3.09 1.055c.698.67.982 1.582 1.097 2.972L22 9H2v-.724c.116-1.39.4-2.302 1.097-2.972s1.645-.944 3.09-1.055V2.724c0-.4.337-.724.753-.724"/><path fill="currentColor" d="M22 14v-2c0-.839-.004-2.335-.017-3H2.01c-.013.665-.01 2.161-.01 3v2c0 3.771 0 5.657 1.172 6.828S6.228 22 10 22h4c3.77 0 5.656 0 6.828-1.172S22 17.772 22 14" opacity="0.5"/><path fill="currentColor" d="M18 17a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-5 4a1 1 0 1 1-2 0a1 1 0 0 1 2 0m0-4a1 1 0 1 1-2 0a1 1 0 0 1 2 0"/></svg>
    ${t_ui('rotationHeader')} ${rotationDimensionCurrent}/${rotationDimensionMax}</strong>
       ...
       <span>${typeof areas[i].unlockDescription === 'function' ? areas[i].unlockDescription() : areas[i].unlockDescription}</span>
        ...
        divAreas.innerHTML = `
                ...
                <span class="explore-ticket-left">
                        ...
                        <span><strong style="background:#9F50B5">${t_ui('levelAbbreviation')} ${Math.max(1,areas[i].level-10)}-${areas[i].level}</strong><span></span></span>
                    </span>
                </div>
                <div style="width: 8rem;" class="explore-ticket-right">
                    <span class="explore-ticket-bg" style="background-image: url(img/bg/${areas[i].background}.png);"></span>
