...
function tooltipData(category, ttdata){
    ...
    if (category == "area") {
        ...
        document.getElementById("tooltipBottom").style.display = "inline";
        document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>${t_ui('areaPokemon')}</strong></div>`;
        ...
        if (areas[ttdata].type == "dungeon") { document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>${t_ui('areaPokemon')} ${uncatchableIcon}</strong></div>`; }
        if (areas[ttdata].uncatchable) { document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>${t_ui('areaPokemon')} ${uncatchableIcon}</strong></div>`; }
        ...
        if (listName === "uncommon") { tag = `<span>${t_ui('uncommon')}</span>`; }
        if (listName === "rare") { tag = `<span>${t_ui('rareExcl')}</span>`; }
        ...
        document.getElementById("tooltipMid").innerHTML = `<div id="area-preview-items"><strong>${t_ui('areaItems')}</strong></div>`;
        ...
        if (item.type=="tm") div.innerHTML = `<img style="scale:2" src="img/items/tm${capitalize(move[item.move].type)}.png">` + tag;
        ...
        if (ttdata == areas.wildlifePark.id) {
            document.getElementById("tooltipBottom").innerHTML += t_ui('wildlifeParkRotate')
        }
    ...
    if (category == "trainer") {
        ...
        document.getElementById("tooltipMid").style.display = "none";
        document.getElementById("tooltipBottom").style.display = "inline";
        document.getElementById("tooltipBottom").innerHTML = `<div id="area-preview-spawns"><strong>${t_ui('teamPreview')}</strong></div><div id="preview-field-effects"></div>`;
        ...
        document.getElementById("tooltipMid").innerHTML = `<div id="area-preview-items"><strong>${t_ui('victoryRewards')}</strong></div>`;
        ...
        if (i.type=="tm") div.innerHTML = `<img style="scale:2" src="img/${meow}/tm${capitalize(move[i.move].type)}.png">` + tag;
        ...
        if (areas[ttdata].itemReward) { //new reward function ill eventually have to convert the rest of the code but for now im quite lazy
            ...
            tooltipMid.innerHTML = `<div id="area-preview-items"><strong>${t_ui('victoryRewards')}</strong></div>`;
            ...
            Object.values(areas[ttdata].itemReward).forEach(reward => {
                ...
                if (isPokemon) {
                    ...
                    imgPath = itemData.type === "tm" 
                        ? `items/tm${capitalize(move[itemData.move].type)}` 
                        : `items/${reward.item}`;
                    scale = 2;
                }
        ...
        if (areas[ttdata].fieldEffect && areas[ttdata].type != "frontier") { 
            document.getElementById("preview-field-effects").innerHTML = t_ui('fieldEffects')
        ...
        if (areas[ttdata].hpPercentage){
            document.getElementById("tooltipBottom").innerHTML += t_ui('timedEncounterDesc');
        }
        ...
        if (areas[ttdata].type == "event" && (areas[ttdata].difficulty == tier3difficulty || areas[ttdata].difficulty == tier4difficulty)){
            ...
            let tag = "";
            if (areas[ttdata].difficulty == tier3difficulty) tag = `<span>${t_ui('uncommon')}!</span>`; 
            div.className = "area-preview";
    ...
    if (category == "buff") {
        ...
        document.getElementById("tooltipBottom").style.display = `inline`
        document.getElementById("tooltipTitle").style.display = `inline`
        if (ttdata==="burn") document.getElementById("tooltipBottom").innerHTML = t_ui('statusBurnDesc')
        if (ttdata==="poisoned") document.getElementById("tooltipBottom").innerHTML = t_ui('statusPoisonDesc')
        if (ttdata==="sleep") document.getElementById("tooltipBottom").innerHTML = t_ui('statusSleepDesc')
        if (ttdata==="freeze") document.getElementById("tooltipBottom").innerHTML = t_ui('statusSleepDesc')
        if (ttdata==="confused") document.getElementById("tooltipBottom").innerHTML = t_ui('statusConfusedDesc')
        if (ttdata==="paralysis") document.getElementById("tooltipBottom").innerHTML = t_ui('statusParalysisDesc')

        if (ttdata==="embargo") document.getElementById("tooltipBottom").innerHTML = t_ui('statusEmbargoDesc')
        
        if (ttdata==="sunny") document.getElementById("tooltipBottom").innerHTML = t_ui('weatherSunnyDesc')
        if (ttdata==="rainy") document.getElementById("tooltipBottom").innerHTML = t_ui('weatherRainyDesc')
        if (ttdata==="sandstorm") document.getElementById("tooltipBottom").innerHTML = t_ui('weatherSandstormDesc')
        if (ttdata==="hail") document.getElementById("tooltipBottom").innerHTML = t_ui('weatherHailDesc')
        if (ttdata==="foggy") document.getElementById("tooltipBottom").innerHTML = t_ui('weatherFoggyDesc')
        if (ttdata==="electricTerrain") document.getElementById("tooltipBottom").innerHTML = t_ui('terrainElectricDesc')
        if (ttdata==="grassyTerrain") document.getElementById("tooltipBottom").innerHTML = t_ui('terrainGrassyDesc')
        if (ttdata==="mistyTerrain") document.getElementById("tooltipBottom").innerHTML = t_ui('terrainMistyDesc')

        if (ttdata==="trickRoom") document.getElementById("tooltipBottom").innerHTML = t_ui('fieldTrickRoomDesc')
        if (ttdata==="weirdRoom") document.getElementById("tooltipBottom").innerHTML = t_ui('fieldWeirdRoomDesc')
        if (ttdata==="crossRoom") document.getElementById("tooltipBottom").innerHTML = t_ui('fieldCrossRoomDesc')
        //if (ttdata==="reflect") document.getElementById("tooltipBottom").innerHTML = `Decreases the damage dealt by physical attacks by 75%`
        if (ttdata==="lightScreen") document.getElementById("tooltipBottom").innerHTML = t_ui('fieldLightScreenDesc')
        if (ttdata==="safeguard") document.getElementById("tooltipBottom").innerHTML = t_ui('fieldSafeguardDesc')
    ...
    for (const i in areas) { //regular areas
        ...
        divAreas.innerHTML = `
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="currentColor" d="M25.719 4.781a2.9 2.9 0 0 0-1.125.344l-4.719 2.5L13.5 6.062l-.375-.093l-.375.187l-2.156 1.25l-1.281.75l1.187.906l2.719 2.063l-3.406 1.813l-3.657-1.657l-.437-.187l-.438.219l-1.75.937l-1.156.625l.875.938l5.406 5.812l.5.594l.688-.375L15 17.094l-1.031 5.687l-.344 1.813l1.719-.719l2.562-1.094l.375-.156l.157-.375l3.718-9.031l5.25-2.813c1.446-.777 2.028-2.617 1.25-4.062a3 3 0 0 0-1.781-1.438a3.1 3.1 0 0 0-1.156-.125m.187 2c.125-.008.254-.004.375.032a.979.979 0 0 1 .188 1.812l-5.594 3.031l-.313.156l-.125.344l-3.718 8.938l-.438.187l1.063-5.906l.375-2.031l-1.813.969l-6.312 3.406l-3.969-4.313l.156-.094l3.657 1.626l.468.218l.406-.25l15.22-8.031a.9.9 0 0 1 .374-.094M13.375 8.094l3.844.937l-2.063 1.063l-2.25-1.719zM3 26v2h26v-2z"/></svg>
                        ...
                        <span><strong style="background:#B18451">${t_ui('levelLabel')}: ${Math.max(1,areas[i].level-10)}-${areas[i].level}</strong><span></span></span>
                    </span>
                </div>
    ...
    div.innerHTML = `
        ...
        <span><strong style="background:#964646ff">${t_ui('hpRemainingLabel')}: ${areas[i].hpPercentage.toFixed(0)}%</strong><span></span></span>
        </span>
        </div>
        <div style="width: 8rem;" class="explore-ticket-right">
    ...
    if (saved.arenaCurrentTrainer==6){
        ...
        document.getElementById("tooltipTitle").style.display = `none`
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = t_ui('allTrainersDefeatedForNow')
        openTooltip()
        return
    }
    ...
    ${t_ui('pickDifficultyNextTrainer')}
    <span>${t_ui('rightClickLongTapFieldDetails')}</span>
    </div>
    ...
    ${t_ui('levelLabel')} 100
    <span>
    ${t_ui('rewardLabel')}: x2<img  src="img/items/goldenBottleCap.png">
    </span>
    </div>
    ...
    ${t_ui('levelLabel')} 125
    <span>
    ${t_ui('rewardLabel')}: x3<img  src="img/items/goldenBottleCap.png">
    </span>
    </div>
    ...
    ${t_ui('levelLabel')} 150
    <span>
    ${t_ui('rewardLabel')}: x4<img  src="img/items/goldenBottleCap.png">
    </span>
    </div>
...
frontierArray.sort((a, b) => a.data.tier - b.data.tier);
    ...
    const i = obj.key;
       ...
       //if (areas[i].reward.includes(item.goldenBottleCap)) nameTag = `<svg class="event-icon" style="color:#465f96" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12.795 2h-2c-1.886 0-2.829 0-3.414.586c-.586.586-.586 1.528-.586 3.414v3.5h10V6c0-1.886 0-2.828-.586-3.414S14.681 2 12.795 2" opacity="0.5"/><path fill="currentColor" fill-rule="evenodd" d="M13.23 5.783a3 3 0 0 0-2.872 0L5.564 8.397A3 3 0 0 0 4 11.031v4.938a3 3 0 0 0 1.564 2.634l4.794 2.614a3 3 0 0 0 2.872 0l4.795-2.614a3 3 0 0 0 1.564-2.634V11.03a3 3 0 0 0-1.564-2.634zM11.794 10.5c-.284 0-.474.34-.854 1.023l-.098.176c-.108.194-.162.29-.246.354s-.19.088-.399.135l-.19.044c-.739.167-1.108.25-1.195.532c-.088.283.163.577.666 1.165l.13.152c.144.167.215.25.247.354s.022.215 0 .438l-.02.203c-.076.785-.114 1.178.116 1.352s.575.015 1.266-.303l.179-.082c.196-.09.294-.135.398-.135s.203.045.399.135l.179.082c.69.319 1.036.477 1.266.303s.192-.567.116-1.352l-.02-.203c-.022-.223-.033-.334 0-.438c.032-.103.103-.187.246-.354l.13-.152c.504-.588.755-.882.667-1.165c-.088-.282-.457-.365-1.194-.532l-.191-.044c-.21-.047-.315-.07-.399-.135c-.084-.064-.138-.16-.246-.354l-.098-.176c-.38-.682-.57-1.023-.855-1.023" clip-rule="evenodd"/></svg>`
        let prefix = t_ui('rookieTrainerPrefix')
        if (areas[i].tier==2) prefix = t_ui('veteranTrainerPrefix')
        if (areas[i].tier==3) prefix = t_ui('aceTrainerPrefix')
        if (areas[i].tier==4) prefix = t_ui('expertTrainerPrefix')
        if (areas[i].tier==5) prefix = t_ui('masterTrainerPrefix')
        ...
        divAreas.innerHTML = `
                ...
                <div class="vs-card-bg"></div>
                    <span class="explore-ticket-left" style="z-index: 2;">
                        ...
                        <span><strong style="font-size:1rem; background:#964646ff">${t_ui('trainerCounter').replace('{current}', saved.arenaCurrentTrainer).replace('{total}', 5)}</strong></span>
                    </span>
                <div>
                </div>
    ...
    for (const i in areas[ttdata].skills) {
        const div = document.createElement("div")
        div.className = `arena-card-info`
        div.innerHTML = `<span>${t_ui('onHPBarDepleted').replace('{number}', i)}</span><div>${skill[areas[ttdata].skills[i]].info()}</div>`
        document.getElementById("arena-card-effect-list").appendChild(div)
    }
    ...
    if (category == "help") {
        ...
        if (ttdata === `VS`) document.getElementById("tooltipTitle").innerHTML = t_ui('vsTrainers')
        if (ttdata === `VS`) document.getElementById("tooltipBottom").innerHTML = t_ui('vsHelp')

        if (ttdata === `Frontier`) document.getElementById("tooltipTitle").innerHTML = t_ui('battleFrontier')
        if (ttdata === `Frontier`) document.getElementById("tooltipBottom").innerHTML = t_ui('frontierHelp')

        if (ttdata === `Spiral`) document.getElementById("tooltipTitle").innerHTML = t_ui('battleTower')
        if (ttdata === `Spiral`) document.getElementById("tooltipBottom").innerHTML = t_ui('spiralHelp')
        if (ttdata === `Spiral`) document.getElementById("tooltipMid").style.display = `inline`
        if (ttdata === `Spiral`) document.getElementById("tooltipMid").innerHTML = t_ui('spiralTypeRotation') + format(saved.currentSpiralingType)


        if (ttdata === `BattleFactory`) document.getElementById("tooltipTitle").innerHTML = t_ui('battleFactory')
        if (ttdata === `BattleFactory`) document.getElementById("tooltipBottom").innerHTML = t_ui('battleFactoryHelp')
        if (ttdata === `BattleFactory`) document.getElementById("tooltipMid").innerHTML = `<div id="area-preview-spawns" data-pkmn="${areas.frontierBattleFactory.icon.id}"><strong>${t_ui('factoryPokemon')}</strong><img class="sprite-trim" src="img/pkmn/sprite/${areas.frontierBattleFactory.icon.id}.png"></div>`;
        if (ttdata === `BattleFactory`) document.getElementById("tooltipMid").style.display = `inline`

        if (ttdata === `BattleArena`) document.getElementById("tooltipTitle").innerHTML = t_ui('battleArena')
        if (ttdata === `BattleArena`) document.getElementById("tooltipBottom").innerHTML = t_ui('battleArenaHelp')

        if (ttdata === `Wild Areas`) document.getElementById("tooltipTitle").innerHTML = t_ui('wildAreas')
        if (ttdata === `Wild Areas`) document.getElementById("tooltipBottom").innerHTML = t_ui('wildAreasHelp')

        if (ttdata === `Dungeons`) document.getElementById("tooltipTitle").innerHTML = t_ui('dungeons')
        if (ttdata === `Dungeons`) document.getElementById("tooltipBottom").innerHTML = t_ui('dungeonsHelp')

        if (ttdata === `Training`) document.getElementById("tooltipTitle").innerHTML = t_ui('training')
        if (ttdata === `Training`) document.getElementById("tooltipBottom").innerHTML = t_ui('trainingHelpFull')

        if (ttdata === `Events`) document.getElementById("tooltipTitle").innerHTML = t_ui('events')
        if (ttdata === `Events`) document.getElementById("tooltipBottom").innerHTML = t_ui('eventsHelpFull')

        if (ttdata === `Dimension`) document.getElementById("tooltipTitle").innerHTML = t_ui('megaDimension')
        if (ttdata === `Dimension`) document.getElementById("tooltipBottom").innerHTML = t_ui('megaDimensionHelp')

        if (ttdata === `Genetics`) document.getElementById("tooltipTitle").innerHTML = t_ui('genetics')
        if (ttdata === `Genetics`) document.getElementById("tooltipBottom").innerHTML = t_ui('geneticsHelpFull')

        if (ttdata === `Pokerus`) document.getElementById("tooltipTitle").innerHTML = t_ui('pokerus')
        if (ttdata === `Pokerus`) document.getElementById("tooltipBottom").innerHTML = t_ui('pokerusHelpFull')

        if (ttdata === `settingsNewPkmn`) document.getElementById("tooltipTitle").innerHTML = t_ui('setting')
        if (ttdata === `settingsNewPkmn`) document.getElementById("tooltipBottom").innerHTML = t_ui('settingsNewPkmnHelp')

        if (ttdata === `settingsAlternateWild`) document.getElementById("tooltipTitle").innerHTML = t_ui('setting')
        if (ttdata === `settingsAlternateWild`) document.getElementById("tooltipBottom").innerHTML = t_ui('settingsAlternateWildHelp')


        if (ttdata === `searchDictionary`) document.getElementById("tooltipTitle").innerHTML = t_ui('keywords')
        if (ttdata === `searchDictionary`) document.getElementById("tooltipBottom").innerHTML = t_ui('keywordsDictionaryHelp')
        if (ttdata === `searchDictionary`) document.getElementById("dictionary-search").blur()


        if (ttdata === `searchPokedex`) document.getElementById("tooltipTitle").innerHTML = t_ui('keywords')
        if (ttdata === `searchPokedex`) document.getElementById("tooltipBottom").innerHTML = t_ui('keywordsPokedexHelp')
        if (ttdata === `searchPokedex`) document.getElementById("pokedex-search").blur()

        if (ttdata === `Main Challenges`) document.getElementById("tooltipTitle").innerHTML = t_ui('mainChallenges')
        if (ttdata === `Main Challenges`) document.getElementById("tooltipBottom").innerHTML = t_ui('mainChallengesHelp')

        if (ttdata === `Custom Challenges`) document.getElementById("tooltipTitle").innerHTML = t_ui('customChallenges')
        if (ttdata === `Custom Challenges`) document.getElementById("tooltipBottom").innerHTML = t_ui('customChallengesHelp')

        if (ttdata === `ModHard`) document.getElementById("tooltipTitle").innerHTML = t_ui('hardMode')
        if (ttdata === `ModHard`) document.getElementById("tooltipBottom").innerHTML = t_ui('hardModeHelp')

        if (ttdata === `ModNuzloke`) document.getElementById("tooltipTitle").innerHTML = t_ui('nuzlokeMode')
        if (ttdata === `ModNuzloke`) document.getElementById("tooltipBottom").innerHTML = t_ui('nuzlokeModeHelp')

        if (ttdata === `ModNoIvs`) document.getElementById("tooltipTitle").innerHTML = t_ui('noIVs')
        if (ttdata === `ModNoIvs`) document.getElementById("tooltipBottom").innerHTML = t_ui('noIVsHelp')

        if (ttdata === `ModAfk`) document.getElementById("tooltipTitle").innerHTML = t_ui('longerAFK')
        if (ttdata === `ModAfk`) document.getElementById("tooltipBottom").innerHTML = t_ui('longerAFKHelp')

        if (ttdata === `curry`) document.getElementById("tooltipTitle").innerHTML = t_ui('curry')
        if (ttdata === `curry`) document.getElementById("tooltipBottom").innerHTML = t_ui('curryHelp')

        if (ttdata === `Shop`) document.getElementById("tooltipTitle").innerHTML = t_ui('shop')
        if (ttdata === `Shop`) document.getElementById("tooltipBottom").innerHTML = t_ui('shopHelp')
    ...
    if (category== "ability") {
        ...
        document.getElementById("tooltipTitle").style.display = `inline`
        document.getElementById("tooltipBottom").style.display = `inline`
        document.getElementById("tooltipMid").innerHTML = t_ui('commonAbility')
        if (ability[ttdata].rarity===2) document.getElementById("tooltipMid").innerHTML = t_ui('uncommonAbility')
        if (ability[ttdata].rarity===3) document.getElementById("tooltipMid").innerHTML = t_ui('rareAbility')
        ...
        if (document.getElementById("pkmn-editor").style.display == "flex" && pkmn[currentEditedPkmn].hiddenAbility?.id == ttdata ) document.getElementById("tooltipBottom").innerHTML += `<br><br>${t_ui('hiddenAbilityDesc')}`
    ...
    if (category=="ribbon") {
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = isFR && ribbon[ttdata].renameFR ? ribbon[ttdata].renameFR : ribbon[ttdata].name
        document.getElementById("tooltipBottom").style.display = `none`
        document.getElementById("tooltipMid").innerHTML = isFR && ribbon[ttdata].descriptionFR ? ribbon[ttdata].descriptionFR : ribbon[ttdata].description
        if (ttdata==`tower1` && pkmn[currentEditedPkmn].recordSpiraling!=undefined) document.getElementById("tooltipMid").innerHTML +=`<br>(${t_ui('highestSpiralingTowerFloor')}: ${pkmn[currentEditedPkmn].recordSpiraling})`
        openTooltip()
    }
    ...
    if (category=="move") {
        ...
        document.getElementById("tooltipTitle").style.display = `inline`
        document.getElementById("tooltipTop").innerHTML = `<img src="img/items/tm${capitalize(move[ttdata].type)}.png">`
        document.getElementById("tooltipTitle").innerHTML = format(ttdata)
        document.getElementById("tooltipMid").style.display = "inline"
        ...
        if (document.getElementById("pkmn-editor").style.display == "flex" && pkmn[currentEditedPkmn].signature?.id == ttdata ) document.getElementById("tooltipBottom").innerHTML += `<br><br>${t_ui('signatureMoveDesc')}`
        if (document.getElementById("pkmn-editor").style.display == "flex" && pkmn[currentEditedPkmn].eggMove?.id == ttdata ) document.getElementById("tooltipBottom").innerHTML += `<br><br>${t_ui('eggMoveDesc')}`
    ...
    if (category=="item") {
        ...
        document.getElementById("tooltipTop").style.display = "flex"
        if (item[ttdata].type == "tm") document.getElementById("tooltipTop").innerHTML = `<img src="img/items/tm${capitalize(move[item[ttdata].move].type)}.png">`
        ...
        if (item[ttdata].type==="held" || item[ttdata].heldBonusPower){
            document.getElementById("tooltipTitle").innerHTML = format(ttdata) + `<br>${t_ui('levelLabel')} ${returnItemLevel(ttdata)}`
            document.getElementById("tooltipMid").innerHTML = `${returnItemLevel(ttdata,"stars")}<br>${item[ttdata].got} ${t_ui('inTotal')} ${returnItemLevel(ttdata,"left")}`
        }

        else document.getElementById("tooltipMid").innerHTML = `${item[ttdata].got} ${t_ui('inBag')}`
    ...
    if (category == "pkmn") {
        ...
        <div style="display: flex;">
            ...
            <div style="box-shadow: none; outline:none; cursor:default">
                        ${t_ui('caughtCount')} ${pkmn[ttdata].caught}
                </div>
    ...
    if (pkmn[poke.id].starsign) document.getElementById("pkmn-editor-sprite").style.filter = `hue-rotate(${starsign[pkmn[poke.id].starsign].hue}deg)`
        ...
        document.getElementById("pkmn-editor-level").innerHTML = `${t_ui('levelLabel')} ${poke.level}`
        ...
        if (pkmn[ttdata].evolve && pkmn[ttdata].evolve()[1].level>0){
            evolutionTag = `<span>${t_ui('unlocksAtLevel').replace('{pokemon}', format(pkmn[ttdata].evolve()[1].pkmn.id)).replace('{level}', pkmn[ttdata].evolve()[1].level)} ❌</span>`
            if (pkmn[ pkmn[ttdata].evolve()[1].pkmn.id ].caught>0) evolutionTag = `<span>${t_ui('unlocksAtLevel').replace('{pokemon}', format(pkmn[ttdata].evolve()[1].pkmn.id)).replace('{level}', pkmn[ttdata].evolve()[1].level)} ✔️</span>`
        }
        ...
        if (pkmn[ pkmn[ttdata].evolve()[evo].pkmn.id ].caught>0) evolutionTag += `<span>${t_ui('unlocksByUsingItem').replace('{pokemon}', format(pkmn[ttdata].evolve()[evo].pkmn.id)).replace('{item}', format(item[pkmn[ttdata].evolve()[evo].item.id].id)).replace('{level}', levelRequired)} ✔️</span>`
        else evolutionTag += `<span>${t_ui('unlocksByUsingItem').replace('{pokemon}', format(pkmn[ttdata].evolve()[evo].pkmn.id)).replace('{item}', format(item[pkmn[ttdata].evolve()[evo].item.id].id)).replace('{level}', levelRequired)} ❌</span>`
        ...
        let natureText = ""
        if (pkmn[ttdata].nature) {
            const isFR = window.gameLang === 'fr';
            natureText =`<span class="nature-text">${isFR ? t_ui('natureSuffix') + ' ' + format(pkmn[ttdata].nature) : format(pkmn[ttdata].nature) + ' ' + t_ui('natureSuffix')}</span>`
        }
        ...
        `
                    ...
                    document.getElementById("pkmn-edit-division").innerHTML =  `<span>${returnDivisionLetter(returnPkmnDivision(poke))}</span> ${t_ui('divisionLabel')}`
    ...
    if (pkmn[ttdata].signature) {
        ...
        document.getElementById("pkmn-edit-signature-text").textContent = t_ui('signatureMoveLabel')
        ...
        document.getElementById("pkmn-stats-stat-signature").style.display = "flex"
        document.getElementById("pkmn-edit-signature-text").textContent = t_ui('eggMoveLabel')
    ...
    document.getElementById("pkmn-edit-ability-hidden").className = ""
    document.getElementById("pkmn-edit-ability-hidden").innerHTML = `<span>${abilityHiddenIcon}${t_ui('wip')}</span>`
    ...
    + format(moveId) + signatureIcon +
     `</span></span><strong>(${move[moveId].power} ${t_ui('moveBasePower')}, ${format(move[moveId].split)})</strong><img style="background: ${returnTypeColor(move[ moveId ].type)} " src="img/icons/${move[ moveId ].type }.svg">`
...
const sortedMovepool = movepool
  ...
  .sort((a, b) => {
    ...
    if ((areas[saved.currentArea]?.trainer && saved.currentArea != undefined ||  areas[saved.currentArea]?.type == "frontier" ||  saved.currentArea == areas.training.id) && pkmn[ttdata].battling==true) {
        ...
        document.getElementById("tooltipTop").style.display = "none"
        document.getElementById("tooltipBottom").style.display = "none"
        document.getElementById("tooltipTitle").innerHTML = t_ui('restrictedAction')
        document.getElementById("tooltipMid").innerHTML = t_ui('restrictedActionDesc')
        openTooltip()
        return
    ...
    if (move[moveId].restricted && saved.currentArea != undefined && pkmn[ttdata].battling==true){
        ...
        document.getElementById("tooltipTop").style.display = "none"
        document.getElementById("tooltipBottom").style.display = "none"
        document.getElementById("tooltipTitle").innerHTML = t_ui('restrictedMove')
        document.getElementById("tooltipMid").innerHTML = t_ui('restrictedMoveDesc')
        openTooltip()
        return
    }
    ...
    class="pkmn-movebox-progress" style="background: ${returnTypeColor(move[ moveId ].type)} "></div><span>`
    + format(moveId) + signatureIcon +
     `</span><strong>(${move[moveId].power} ${t_ui('moveBasePower')}, ${format(move[moveId].split)})</strong><img style="background: ${returnTypeColor(move[ moveId ].type)} " src="img/icons/${move[ moveId ].type }.svg">`
    ...
    if (category == "dictionaryAbility") {
        ...
        document.getElementById("tooltipTop").style.display = `none`
        document.getElementById("tooltipTitle").innerHTML = format(ttdata)
        document.getElementById("tooltipMid").innerHTML = `${t_ui('commonAbility')}<br>${ability[ttdata].info()}`
        if (ability[ttdata].rarity===2) document.getElementById("tooltipMid").innerHTML = `${t_ui('uncommonAbility')}<br>${ability[ttdata].info()}`
        if (ability[ttdata].rarity===3) document.getElementById("tooltipMid").innerHTML = `${t_ui('rareAbility')}<br>${ability[ttdata].info()}`
        ...
        if (signatureArray.length>0) signatureText = `<br><br>${t_ui('additionallyHiddenAbilityOf').replace('{pokemon}', joinWithAnd(signatureArray))}`
        

        document.getElementById("tooltipBottom").innerHTML = `
        ${t_ui('learnableByTypes').replace('{types}', joinWithAnd(ability[ttdata].type)).replace('{signature}', signatureText)}
        `
        } else {
        ...
        document.getElementById("tooltipBottom").innerHTML = `
        ${t_ui('hiddenAbilityOf').replace('{pokemon}', joinWithAnd(signatureArray))}
        `
        } else{
        document.getElementById("tooltipBottom").innerHTML = `
        ${t_ui('abilityCurrentlyUnobtainable')}
        ` 
        }
   ...
   if (category == "dictionaryMove") {
        ...
        document.getElementById("tooltipMid").innerHTML = `${move[ttdata].power} ${t_ui('powerLabel')}, ${format(move[ttdata].split)}${affectedText}`
        ...
        let rarity = `${t_ui('commonParen')}`
        if (move[ttdata].rarity == 2) rarity = `${t_ui('uncommonParen')}`
        if (move[ttdata].rarity == 3) rarity = `${t_ui('rareParen')}`

        document.getElementById("tooltipBottom").innerHTML = `
        ${t_ui('learnableByTypes').replace('{types}', joinWithAnd(move[ttdata].moveset)).replace('{signature}', rarity)}
        `
        } else{
        ...
        document.getElementById("tooltipBottom").innerHTML = `
        ${t_ui('signatureMoveOf').replace('{pokemon}', joinWithAnd(signatureArray))}
        `
        if (eggMoveUsers.length>0) document.getElementById("tooltipBottom").innerHTML += `
        ${t_ui('eggMoveOf').replace('{pokemon}', joinWithAnd(eggMoveUsers))}
        `
    ...
    if (category == "dictionaryPkmn") {
        ...
        let spawnLocation = ""
        if (wildSpawn != "") spawnLocation += `<span>${t_ui('foundInWildArea').replace('{area}', format(wildSpawn)).replace('{rotation}', areas[wildSpawn].rotation)}</span>`
        if ( wildlifePoolCommon.includes(ttdata) || wildlifePoolUncommon.includes(ttdata) || wildlifePoolRare.includes(ttdata) ) spawnLocation += `<span>${t_ui('foundInWildlifePark')}</span>`
        if ( exclusiveFrontierPkmn.includes(pkmn[ttdata]) ) spawnLocation += `<span>${t_ui('obtainedBattleFrontierRandom')}</span>`
        if (encounterSpawn != "") spawnLocation += `<span>${t_ui('obtainedInEvent').replace('{event}', format(areas[encounterSpawn].name)).replace('{rotation}', areas[encounterSpawn].rotation)}</span>`
        if (eventSpawn != "") spawnLocation += `<span>${t_ui('foundInEvent').replace('{event}', format(eventSpawn)).replace('{rotation}', areas[eventSpawn].rotation)}</span>`
        if (spawnLocation == "") spawnLocation = t_ui('pokemonCannotBeCaughtStage')
        if (pkmn[ttdata].tagObtainedIn == "unobtainable") spawnLocation = t_ui('pokemonUnobtainable')
        if (pkmn[ttdata].tagObtainedIn == "mart") spawnLocation = t_ui('canBePurchasedInPokeMart')
        if (pkmn[ttdata].tagObtainedIn == "arceus") spawnLocation = t_ui('unlockedAllObtainableCaught')
        ...
        if (saved.gamemodAfk !=true) document.getElementById("tooltipMid").innerHTML += t_ui('lastTurnsBeforeFainting').replace('{turns}', (100 + Math.pow(1.15, 6) * (pkmn[ttdata].bst.hp*30 + (pkmn[ttdata].bst.def + pkmn[ttdata].bst.sdef)*15)*staminaMult).toFixed(0))
        else document.getElementById("tooltipMid").innerHTML += t_ui('lastTurnsBeforeFainting').replace('{turns}', (100 + Math.pow(1.15, 6) * (pkmn[ttdata].bst.hp*30 + (pkmn[ttdata].bst.def + pkmn[ttdata].bst.sdef)*15)*staminaMult).toFixed(0)*3)


        document.getElementById("tooltipBottom").innerHTML = `
        <div>${t_ui('abilitiesSection')}</div>
        ...
        <div onclick="window.getMoveset(pkmn.${ttdata})" class="custom-challenge-button" style="margin-top:0.5rem">${t_ui('checkLearnableMoves')}</div>
    ...
    if (category == "dictionaryItem") {
        ...
        document.getElementById("tooltipMid").innerHTML = `${item[ttdata].info()}<br>(${t_ui('youHave').replace('{count}', item[ttdata].got)})`
        ...
        for (const i in areas){
            ...
            if (areas[i].drops?.common?.includes(item[ttdata]) || areas[i].drops?.uncommon?.includes(item[ttdata]) || areas[i].drops?.rare?.includes(item[ttdata])){
                obtainText += `<span>${t_ui('canBeDroppedWildArea').replace('{area}', format(i)).replace('{rotation}', areas[i].rotation)}</span>`
            }
            }

            if (areas[i].type=="event") {
            if (areas[i].drops?.common?.includes(item[ttdata]) || areas[i].drops?.uncommon?.includes(item[ttdata]) || areas[i].drops?.rare?.includes(item[ttdata])){
                obtainText += `<span>${t_ui('canBeDroppedEventArea').replace('{area}', format(i)).replace('{rotation}', areas[i].rotation)}</span>`
            }
            }

            if (areas[i].type=="dungeon") {
            if (areas[i].drops?.common?.includes(item[ttdata]) || areas[i].drops?.uncommon?.includes(item[ttdata]) || areas[i].drops?.rare?.includes(item[ttdata])){
                obtainText += `<span>${t_ui('canBeDroppedDungeonArea').replace('{area}', format(i)).replace('{rotation}', areas[i].rotation)}</span>`
            }
            }
            ...
            if (areas[i].reward?.includes(item[ttdata])){
                obtainText += `<span>${t_ui('rewardFromEvent').replace('{event}', format(areas[i].name)).replace('{rotation}', areas[i].rotation)}</span>`
            }
            }
        }

        if ([item.hpUp.id, item.protein.id, item.iron.id, item.calcium.id, item.zinc.id, item.carbos.id, item.timeCandy.id].includes(ttdata)) obtainText += `<span>${t_ui('obtainedViaExportReward')}</span>`

        for (const i in spiralingRewards) if (spiralingRewards[i].item==ttdata) obtainText += `<span>${t_ui('rewardFromSpiralingTower')}</span>`


        for (const i in shop) {
            if (shop[i].icon == ttdata) obtainText += `<span>${t_ui('canBeBoughtInPokeMart')}</span>`
        }

        if (item[ttdata].type=="decor" && item[ttdata].rarity) obtainText += `<span>${t_ui('canBeBoughtInPokeMart')}</span>`


        if (obtainText=="") obtainText += `<span>${t_ui('itemCurrentlyUnobtainable')}</span>`
